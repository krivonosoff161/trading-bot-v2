...
The current task branch carries a non-authoritative candidate that preserves
...
typed, resumable backfill after the completed product checkpoint. Its partial
identity-bound reject cache is an accelerator, never a published complete memory
snapshot. A cache miss, corruption, classifier/schema mismatch or interruption
therefore cannot silently become complete memory or weaken current product
truth. The branch also adds a canonical crash-safe final-evidence seal protocol;
...
1. **Startup-memory critical path:** merge and independently verify the
   non-authoritative cold-cache repair, then prove a completely cold historical
   backfill cannot delay T+0 while its later progress remains observable.
