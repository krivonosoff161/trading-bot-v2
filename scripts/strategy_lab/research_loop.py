...
_HARD_ITER_CAP = (
    10_000  # backstop; the loop is bounded by duration/max-iterations anyway
)
_MAX_DURATION_MIN = 240  # quiet desktop backstop ceiling
_NIGHT_MAX_DURATION_MIN = 720
_IDLE_FLOOR_SECONDS = (
    60  # when apply-mode work is throttled/idle, wait at least this (no busy-spin)
)
...
def _build_summary(private_root: Path) -> str:
    ...
    return (
        f"candidates={reg.get('entries', 0)} (unique {reg.get('unique_candidates', 0)}), "
...
def _provider_error_payload(exc: LLMProviderError, provider) -> dict:
    ...
    next_action = (
        ...
        if retryable
        else "non-retryable; check provider/model availability and config before the next run"
    )
    return {
...
def _llm_step(args, private_root, universe, profiles, policy) -> dict:
    ...
    provider = load_provider(
        ...
        synthetic_candidates=_synthetic_candidates(universe),
    )
    ...
    cfg = load_llm_loop_config(
        max_candidates=max(1, args.max_candidates), allow_synthetic=allow_synth
    )
    spent = today_usage(private_root)["cost_rub"]
    if cfg.daily_cap_value is not None and spent >= cfg.daily_cap_value:
        return {
            "status": "daily_cap_exhausted",
    ...
    if provider.name not in {
        "synthetic",
        "ollama",
    }:  # paid/network provider needs all spend gates
        gate = evaluate_llm_loop_gates(
            cfg, send_requested=True, dry_run=False, spent_today=spent
        )
        if not gate.allowed:
    ...
    try:
        batch, usage = generate_proposals_via_llm(
            provider,
            summary=_build_summary(private_root),
            universe=universe,
        ...
        iterations.append(
            {
                "iteration": i,
                "llm": llm,
                "queued": c.get("proposals_queued", 0),
                "ready": c.get("ready_jobs", 0),
                "missing": c.get("skipped_missing_data", 0),
                "worker": {
                    "completed": c.get("worker_completed", 0),
                    "deferred": c.get("worker_deferred", 0),
...
def _final_report(mode, args, iterations, totals, llm_cost, start) -> dict:
    return {
        ...
        "safety": {
            ...
            "no_order_engine": True,
            "no_paid_llm_default": True,
...
def _write_running_report(
    private_root, mode, args, iterations, totals, llm_cost, start, allow_public
) -> None:
    write_loop_report(
        private_root,
        _final_report(mode, args, iterations, totals, llm_cost, start),
...
def _print(report: dict) -> None:
    t = report["totals"]
    print(
        f"Strategy Lab research loop ({report['mode'].upper()}) - {report['iterations']} iteration(s), "
        f"{report['duration_minutes']} min"
    )
    print(
        f"  proposals queued={t['proposals_queued']} ready={t['ready_jobs']} missing_data={t['skipped_missing_data']}"
    )
    print(
        f"  LLM validated={t['llm_validated']} rejected={t['llm_rejected']} cost_rub={report['llm_cost_rub']}"
    )
    print(
        f"  worker completed={t['worker_completed']} deferred={t['worker_deferred']} failed={t['worker_failed']}"
    )
    print(
        "  safety: single bounded run; no live trading, no order engine, no paid LLM by default"
    )
...
def main() -> None:
    ...
    mode.add_argument(
        "--dry-run",
        action="store_true",
        help="No writes (except status), no worker, no network, no LLM (default)",
    )
    mode.add_argument(
        "--apply",
        action="store_true",
        help="Queue data-ready proposals, run capped worker per iteration",
    )
    ...
    ap.add_argument(
        "--llm-propose",
        action="store_true",
        help="Ask the cheap LLM for candidates (gated; dry-run never calls)",
    )
    ap.add_argument(
        "--max-llm-contract-failures",
        type=int,
        default=3,
    ...
    ap.add_argument(
        "--load-env",
        default="",
        help="Optional .env file to load before provider resolution",
    )
    ap.add_argument(
        "--night-mode",
        action="store_true",
        help="Use opt-in night resource policy and longer duration cap",
    )
    ap.add_argument("--prepare-missing-data", action="store_true")
    ap.add_argument(
        "--provider",
        default="null",
        choices=["null", "synthetic", "okx-public"],
        help="DATA provider for prepare",
    )
