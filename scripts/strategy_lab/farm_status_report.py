...
def _farm_loop_status(
    ...
    details = (
        raw_details if isinstance(raw_details := data.get("details"), dict) else {}
    )
    ...
    fresh = bool(
        updated_at and age_seconds is not None and age_seconds <= freshness_limit
    )
...
def collect(
    ...
    *,
    ...
    try:
        unique_table = (
            "farm_results" if _has_rows(conn, "farm_results") else "candidates"
        )
        report: dict = {
            "exists": True,
            ...
            "schema_version": int(
                _scalar(conn, "SELECT value FROM meta WHERE key='schema_version'", 0)
            ),
            "totals": {
                ...
                "unique_candidates": int(
                    _scalar(
                        conn,
                        f"SELECT COUNT(DISTINCT symbol || '|' || family) FROM {unique_table}",
                    )
                ),
            },
            "queue": _counts(
                conn, "SELECT status, COUNT(*) FROM queue GROUP BY status"
            ),
            "queue_age": {
                "oldest_queued": _scalar(
                    conn,
                    "SELECT MIN(created_at) FROM queue WHERE status='queued'",
                    None,
                ),
                "newest_queued": _scalar(
                    conn,
                    "SELECT MAX(created_at) FROM queue WHERE status='queued'",
                    None,
                ),
            },
            "latest_run": dict(
                conn.execute(
                    "SELECT run_id, created_at FROM runs ORDER BY run_id DESC LIMIT 1"
                ).fetchone()
                or {}
            ),
            "decisions": _counts(
                conn, "SELECT decision, COUNT(*) FROM candidates GROUP BY decision"
            ),
            "validation": _counts(
                conn,
                "SELECT validation_status, COUNT(*) FROM candidates GROUP BY validation_status",
            ),
        ...
        if _has_rows(conn, "farm_results"):
            ...
            report["by_timeframe"] = _counts(
                conn, "SELECT timeframe, COUNT(*) FROM farm_results GROUP BY timeframe"
            )
            report["data_quality"] = _counts(
                conn,
                "SELECT data_quality, COUNT(*) FROM farm_results GROUP BY data_quality",
            )
            report["handoff"] = {
                "validation_exported": int(
                    _scalar(
                        conn,
                ...
                "paper_outcomes": int(
                    _scalar(conn, "SELECT COUNT(*) FROM paper_outcomes")
                ),
                "gpu_signal_rows": int(
                    _scalar(
                        conn,
        ...
        if fast:
            ...
            try:
                ...
                report["setup_lifecycle"] = summarize_setup_lifecycle(
            ...
            try:  # Setup Outcome Memory: rejected-as-knowledge sub-views (derived, research-only)
                ...
                report["outcome_memory"] = {
                    **summarize_memory(build_memory_index(db_path.parent.parent)),
                    "product_paper_memory": summarize_product_training_memory(
                        db_path.parent.parent
                    )["summary"],
                }
            except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # Exit Phase-2 dynamic-exit re-sim summary (research-only)
            ...
            data = (
                json.loads(deriv.read_text(encoding="utf-8")) if deriv.exists() else {}
            )
        ...
        try:  # OI-family research summary (separate oi_* namespace; 1h/4h only)
            oi_f = (
                db_path.parent.parent / "state" / "derived" / "oi_family_research.json"
            )
            report["oi_family"] = (
                (json.loads(oi_f.read_text(encoding="utf-8")).get("summary") or {})
                if oi_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # bounded OOS / shadow-forward verdict on survivors (held-out-tail pseudo-OOS, research-only)
            ...
            report["shadow_oos"] = (
                (json.loads(oos_f.read_text(encoding="utf-8")).get("summary") or {})
                if oos_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # true-forward collector (new bars, not held-out tail) — pending until data-prepare fetches
            ...
            report["true_forward"] = (
                (json.loads(tf_f.read_text(encoding="utf-8")).get("summary") or {})
                if tf_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # tactical-probe characterization of sub-power net-positive setups (research-only, not edge)
            ...
            report["tactical_probe"] = (
                (json.loads(tp_f.read_text(encoding="utf-8")).get("probe") or {})
                if tp_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # 15m OI delta_coarse diagnostic bucket (never edge)
            od_f = (
                db_path.parent.parent / "state" / "derived" / "oi_diagnostic_15m.json"
            )
            report["oi_diagnostic_15m"] = (
                (json.loads(od_f.read_text(encoding="utf-8")).get("summary") or {})
                if od_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # latest bounded discovery cycle (live universe -> validate -> tactical -> exit -> memory)
            ...
            report["discovery_cycle"] = (
                json.loads(dc_f.read_text(encoding="utf-8")) if dc_f.exists() else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
            report["discovery_cycle"] = {}
        try:  # tactical track (parallel verdict lane; NO_EVENT != bad; leads = forward-watch, never paper-ready)
            ...
            report["tactical_track"] = (
                (json.loads(tt_f.read_text(encoding="utf-8")).get("summary") or {})
                if tt_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # Theme 40 microstructure lane (separate research-only lane; tape replay + orderbook recorder)
            ...
            report["microstructure"] = (
                (json.loads(mm_f.read_text(encoding="utf-8")).get("summary") or {})
                if mm_f.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        if fast:
            ...
            try:  # the owner's six knowledge-base counts in one line (gate + survived + tactical + recyclable)
                ...
                report["knowledge_base"] = knowledge_base_counts(
                    build_memory_index(db_path.parent.parent),
                    gate_idx,
                    survived_shadow=len(
                        (report.get("shadow_oos") or {}).get("survived") or []
                    ),
                    tactical_probe=int(
                        (report.get("tactical_probe") or {}).get("thin_positive") or 0
                    ),
                )
            except Exception:  # noqa: BLE001 - optional derived view must not break status
        ...
        try:  # completion verdict + two-DB reconciliation (read-only, T2)
            from src.research_lab.farm_reconcile import (
                completion_verdict,
                reconcile_dbs,
            )
        ...
        try:  # decode blocked/deferred tails into structural reasons (read-only)
            from src.research_lab.farm_tasks_db import FarmTasksDB, tasks_db_path
            from src.research_lab.tail_diagnostics import (
            ...
            report["tail_diagnostics"] = summarize_tails(
                blocked, deferred, uni, needs_oi
            )
        ...
        try:  # operational paper-watch lane snapshot (read-only surface)
            ...
            report["paper_signals"] = (
                json.loads(ps_path.read_text(encoding="utf-8"))
                if ps_path.exists()
                else {}
            )
        ...
        try:  # main-readable paper instruction view (derived, never orders)
            ...
            report["main_paper_bridge"] = (
                json.loads(mp_path.read_text(encoding="utf-8"))
                if mp_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # paper-only consumer audit for main-readable instructions (never orders)
            mc_path = (
                db_path.parent.parent / "state" / "derived" / "main_paper_consumed.json"
            )
        ...
        try:  # adaptive policy selected for main paper runtime rows (never prices/orders)
            ap_path = (
                ...
                / "state"
                / "derived"
                / "main_adaptive_policy.json"
            )
            report["main_adaptive_policy"] = (
                json.loads(ap_path.read_text(encoding="utf-8"))
                if ap_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # paper-only runtime observation of the main-compatible queue (never orders)
            ...
            report["main_paper_runtime_observation"] = (
                json.loads(rto_path.read_text(encoding="utf-8"))
                if rto_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # validated main-paper trade ledger (never orders)
            tr_path = (
                db_path.parent.parent / "state" / "derived" / "main_paper_trades.json"
            )
        ...
        try:  # broad subscriber-facing paper product ledger (never orders)
            ...
            report["paper_product_trade_ledger"] = (
                json.loads(ptr_path.read_text(encoding="utf-8"))
                if ptr_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # offline Telegram-card preview for paper-watch instructions (never sends)
            ...
            report["paper_telegram_preview"] = (
                json.loads(pt_path.read_text(encoding="utf-8"))
                if pt_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # Telegram delivery audit for paper cards; reading this never sends.
            ...
            report["paper_telegram_delivery"] = (
                json.loads(pd_path.read_text(encoding="utf-8"))
                if pd_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # Aggregate paper-product quality digest; raw private rows stay in the derived report.
            pq_path = (
                ...
                / "state"
                / "derived"
                / "paper_product_quality_report.json"
            )
            report["paper_product_quality"] = (
                json.loads(pq_path.read_text(encoding="utf-8"))
                if pq_path.exists()
                else {}
            )
        except Exception:  # noqa: BLE001 - optional surface must not break status
        ...
        try:  # PFR bridge: how many canonical records survive quality + risk gates
            ...
            report["pfr_bridge"] = {
                ...
                "unique_setups": len(
                    {(r["symbol"], r["timeframe"], r["family"]) for r in pfr_passed}
                ),
                "risk_too_wide": sum(
                    1
                    for r in pfr_passed
...
def _print(report: dict) -> None:
    if not report.get("exists"):
        print(
            f"no farm DB yet at {report.get('db')} - run the farm first: "
    ...
    print(
        f"FARM STATUS ({mode}) - schema v{report['schema_version']} | runs={t['runs']} candidates={t['candidates']} "
    ...
    print(
        f"  queue: {report['queue'] or '(empty)'}  age[{report['queue_age']['oldest_queued'] or '-'} .. "
    ...
    print(
        f"  flow coverage: {fc.get('funding') if fc.get('tracked') else 'not tracked yet'}"
    )
    ...
    if lc.get("available"):
        ...
        print(
            f"    intake_unconsumed={lc.get('intake_unconsumed', 0)} "
    ...
    if comp.get("available"):
        ...
        if comp.get("state") != "DRAINED":
            ...
            line += (
                f" - eligible_now={rs.get('eligible_now', 0)} running={rs.get('running', 0)} "
    ...
    for name, info in (rc.get("orphans") or {}).items():
        print(
            f"  reconcile orphan: {name}={info.get('count')} owner={info.get('owner')} "
    ...
    for kind in ("provider_error", "too_short"):
        for reason, syms in (td.get(kind) or {}).items():
            symbol_preview = ", ".join(syms[:6]) + (
                f" +{len(syms) - 6}" if len(syms) > 6 else ""
            )
            print(f"  tail {kind}: {reason} -> [{symbol_preview}]")
    if td.get("needs_oi", {}).get("count"):
        oi = td["needs_oi"]
        print(
            f"  tail needs_oi: {oi['reason']} (n={oi['count']}) -> {oi['next_action']}"
        )
    ps = report.get("paper_signals") or {}
    if ps.get("total"):
        print(
            f"  paper signals (operational watch lane, research-only NOT orders): total={ps['total']} "
    ...
    if tl.get("trades") is not None:
        ...
        money = (
            raw_money if isinstance(raw_money := tl.get("paper_money"), dict) else {}
        )
        if money:
            print(
                "    main paper money: "
    ...
    if ptl.get("trades") is not None:
        money = (
            raw_money if isinstance(raw_money := ptl.get("paper_money"), dict) else {}
        )
        print(
            "  paper product trade ledger: "
    ...
    if pq.get("schema") == "paper_product_quality_report.v1":
        ...
        if family_preview:
            print(f"    paper family quality: {'; '.join(family_preview)}")
        raw_geometry_profiles = pq.get("geometry_profiles")
        geometry_profiles = (
            raw_geometry_profiles if isinstance(raw_geometry_profiles, list) else []
        )
        ...
        lifecycle = (
            raw_lifecycle
            if isinstance(raw_lifecycle := pq.get("active_signal_lifecycle"), dict)
            else {}
        )
        if lifecycle:
            print(
                "    paper active lifecycle: "
        ...
        pfr_state = (
            raw_pfr_state
            if isinstance(raw_pfr_state := pq.get("pfr_trigger_state"), dict)
            else {}
        )
        if pfr_state:
            print(
                "    PFR live-trigger state: "
        ...
        pfr_funnel = (
            raw_pfr_funnel
            if isinstance(raw_pfr_funnel := pq.get("pfr_funnel"), dict)
            else {}
        )
        near_reasons = (
            pfr_funnel.get("near_trigger_counts")
            if isinstance(pfr_funnel, dict)
            else {}
        )
        if near_reasons:
    ...
    if pfr_snap.get("records_loaded") is not None:
        print(
            f"  PFR bridge: records_loaded={pfr_snap['records_loaded']} "
        ...
        if pfr_snap.get("risk_too_wide"):
            print(
                f"    geometry gate: stop_pct>MAX_RISK_PCT rejected_approx={pfr_snap['risk_too_wide']} "
                f"(exact gate runs at signal-time)"
            )
        print(
            "    activation: farm_loop --run-paper-signals --pfr-db-path <path>  "
            "OR  paper_signals_run --pfr-db-path <path>"
        )
        print(
            "    NOTE: PFR = farm backtest validated; forward observation only, NOT edge, NOT order"
        )
    sl = report.get("setup_lifecycle") or {}
    if sl.get("available"):
        print(
            f"  setup lifecycle: total={sl.get('total', 0)} states={sl.get('by_state') or '(none)'}"
        )
        if sl.get("by_tactical"):
            print(
                f"    tactical shelf (research-only, never tradeable): {sl['by_tactical']}"
            )
        print(
            f"    paper groups: positive={sl.get('positive_setups', 0)} "
    ...
    if om.get("total"):
        print(
            f"  outcome memory (rejected-as-knowledge, research-only): total={om['total']} "
            f"by_class={om.get('by_outcome_class') or '(none)'}"
        )
        print(
            f"    sub-DBs: positive={om.get('positive', 0)} recovered={om.get('recovered', 0)} "
        ...
        if om.get("revalidated"):
            print(
                f"    re-validated={om['revalidated']} survivors={om.get('revalidation_survivors', 0)} "
                "(survivor = research-only, needs human GO + OOS, never auto paper-ready)"
            )
        if om.get("by_cost_class"):
            print(
                f"    tactical library: cost={om.get('by_cost_class')} "
        ...
        if om.get("paper_memory_rows"):
            print(
                f"    paper memory: rows={om.get('paper_memory_rows', 0)} "
        ...
        product_memory = (
            raw_product_memory
            if isinstance(raw_product_memory := om.get("product_paper_memory"), dict)
            else {}
        )
        if product_memory:
            print(
                f"    product paper memory: rows={product_memory.get('rows', 0)} "
    ...
    if sh.get("shadow_candidates"):
        print(
            f"  shadow-forward watch: candidates={sh['shadow_candidates']} by_family={sh.get('by_family')} "
            f"(forward-only, no execution; all_research_only={sh.get('all_research_only')})"
        )
    ep = report.get("exit_phase2") or {}
    if ep.get("evaluated"):
        print(
            f"  exit phase-2 (mean_rev, research-only): evaluated={ep['evaluated']} by_class={ep.get('by_class')} "
    ...
    if oif.get("evaluated"):
        print(
            f"  oi families (1h/4h only, separate oi_* class): evaluated={oif['evaluated']} "
        ...
        if om.get("paper_ready_without_hard_pass"):
            print(
                f"    WARNING invariant breach: {om['paper_ready_without_hard_pass']} rows "
                "paper_forward_ready WITHOUT a hard PAPER_FORWARD_READY verdict"
            )
    so = report.get("shadow_oos") or {}
    if so.get("evaluated"):
        print(
            f"  shadow OOS (held-out-tail, research-only): evaluated={so['evaluated']} "
    ...
    if tf.get("watched"):
        print(
            f"  true-forward (new bars, research-only): watched={tf['watched']} "
    ...
    if tp.get("thin_total"):
        print(
            f"  tactical probe (n<{tp.get('power_floor')}, research-only): thin={tp['thin_total']} "
    ...
    if od.get("evaluated"):
        print(
            f"  oi 15m DIAGNOSTIC (delta_coarse, never edge): evaluated={od['evaluated']} "
    ...
    if dc.get("steps"):
        ...
        print(
            f"  discovery cycle (latest, research-only): steps_ok={oks}/{len(dc['steps'])} "
            f"held_oos={len(worked)} {worked[:1]}"
        )
    tt = report.get("tactical_track") or {}
    if tt.get("total"):
        print(
            f"  tactical track (parallel lane, never paper-ready): leads={tt.get('tactical_leads')} "
    ...
    if mic:
        ...
        print(
            f"  micro lane (Theme 40, research-only): tape={tape.get('events')}ev "
    ...
    if kb:
        if kb.get("skipped") == "fast_mode":
            print(
                "  KNOWLEDGE BASE (research-only): skipped in --fast (run without --fast for audit rebuild)"
            )
        else:
            print(
                f"  KNOWLEDGE BASE (research-only): known_bad={kb.get('known_bad')} revisit={kb.get('revisit')} "
    ...
    for b in report.get("backend", []):
        print(
            f"  backend eff={b['effective_backend'] or '?'} signal={b['signal_backend'] or '?'} "
    ...
    if rows:
        ...
        for r in rows[:30]:
            print(
                f"    {r['asset_group'] or '-':16s} {r['family']:32s} {r['decision']:24s} "
    ...
    extra = (
        f" | total eligible groups={total_groups} unvalidated_rows={unval}"
    ...
    for r in ready[:12]:
        print(
            f"    {r['symbol']} {r['family']} {r.get('timeframe') or ''} -> {r['validation_status']}"
        )
    if not ready:
        print(
            "    (none yet - all candidates rejected/observe; expected on weak/synthetic data)"
        )
