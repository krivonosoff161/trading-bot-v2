...
def _get_oi_delta(oi_history: list, ts_ms: int) -> float:
    ...
    def _parse(entry):
        if isinstance(entry, dict):
            return int(entry.get("ts", 0)), float(
                entry.get("oi", 0) or entry.get("oiCcy", 0)
            )
...
def _fast_hold_minutes(sig: dict) -> int:
    ...
    if sig.get("trade_style") == "FAST" and sig.get("regime") in (
        "DRIFT",
        "WEAK_TREND",
    ):
...
def _calc_basis_and_perp_div(
    mark_15m: list, index_15m: list, last_price: float, raw_15m: list
) -> tuple:
...
BT_REGIME_SCORE = (
    os.getenv("BT_REGIME_SCORE", "0") == "1"
)  # multi-factor regime upgrade
BT_REGIME_SCORE_MIN = int(
    os.getenv("BT_REGIME_SCORE_MIN", "3")
)  # min score to upgrade RANGING→DRIFT
BT_ADX_PERIOD = int(
    os.getenv("BT_ADX_PERIOD", "9")
)  # 9=fast reaction (default), 14=standard
...
BT_DRIFT_TP1_K = float(
    os.getenv("BT_DRIFT_TP1_K", "0.4")
)  # TP1 multiplier (live: 0.4R)
BT_DRIFT_MAX_VOL = float(
    os.getenv("BT_DRIFT_MAX_VOL", "999")
)  # vol_ratio cap (999 = no cap)
BT_DRIFT_BTC_VOL_MAX = float(
    os.getenv("BT_DRIFT_BTC_VOL_MAX", "3.0")
)  # prod D3: >3.0 veto
BT_DRIFT_VWAP_STRETCH = float(os.getenv("BT_DRIFT_VWAP_STRETCH", "999.0"))
BT_DRIFT_ETH_BLOCK_HOURS = [
    int(h)
    for h in os.getenv("BT_DRIFT_ETH_BLOCK_HOURS", "22,23,0,1").split(",")
    if h.strip().isdigit()
]  # prod B3: hours veto
BT_DRIFT_MOVE_FROM_BASE = float(os.getenv("BT_DRIFT_MOVE_FROM_BASE", "999.0"))
BT_DRIFT_VOL_DECAY_MIN = float(
    os.getenv("BT_DRIFT_VOL_DECAY_MIN", "0.9")
)  # prod D2: <0.9 veto
BT_LATE_SCORE_MAX = int(os.getenv("BT_LATE_SCORE_MAX", "99"))
BT_HOUR_ANALYSIS = os.getenv("BT_HOUR_ANALYSIS", "0") == "1"
BT_TRENDING_REQUIRE_FVG = (
    os.getenv("BT_TRENDING_REQUIRE_FVG", "1") == "1"
)  # prod: trending_require_fvg=true
...
def detect_regime(
    ...
    di_spread_1h: float,
    bb_width: float,
    adx_1h_rising: bool = False,
    st_1h_dir: str = "",
    di_spread_15m: float = 0.0,
) -> str:
    """
...
def compute_signal(
    ...
    if not raw_4h or len(raw_4h) < 50:
        return None  # need 50 bars for calc_ema(c4h, 50)
    if not raw_1h or len(raw_1h) < 50:
        return None  # need 50 bars for calc_ema(c1h, 50)
    ...
    adx_4h_prev, pdi_4h_c, mdi_4h_c = calc_adx(
        h4h, l4h, c4h, period=ADX_PERIOD, bar_index=-2
    )
    ...
    four_h_conflict = (
        bias_4h != "NEUTRAL" and bias_4h != bias_1h and di_spread_4h_closed >= 12
    )
    ...
    five_m_short = True
    _cross_fresh = True  # cross happened ≤ 2 bars ago
    _body_quality = True  # trigger candle body/range > 0.55
    _vol_ratio_5m = 1.0  # 5m volume ratio
    if raw_5m and len(raw_5m) >= 22:
        ...
        if five_m_long:
            ...
            _cross_fresh = any(not above_hist[i] for i in range(1, n_fresh + 1))
        else:
            # Current is below EMA — look back for a bar that was above
            _cross_fresh = any(above_hist[i] for i in range(1, n_fresh + 1))
        ...
        op = o5[-1]  # newest open
        rng = hi - lo
        _body_quality = (abs(trigger_close - op) / rng > 0.55) if rng > 0 else False
        ...
        _vol_ratio_5m = rec5 / pri5 if pri5 > 0 else 1.0
    ...
    day_start_ms = int(
        datetime(
            dt_last.year, dt_last.month, dt_last.day, tzinfo=timezone.utc
        ).timestamp()
        * 1000
    )
    ...
    if BT_REGIME_SCORE and regime == "RANGING":
        rs = 0
        if di_spread_1h >= 8:
            rs += 1  # DI diverging (reacts before ADX)
        if abs(slope_1h) >= 25:
            rs += 1  # price slope strong
        if bias_1h in ("UP", "DOWN"):
            rs += 1  # EMA 20/50 aligned
        if vol_ratio >= 0.8:
            rs += 1  # volume present
        if adx_1h >= 20:
            rs += 1  # ADX not flat (even if lagging)
    ...
    if regime == "CHOPPY":
        ...
        swing_base = (
            float(adx_1h) >= cfg_sw.get("adx", 18)
            and adx_1h_rising
            and vol_ratio >= cfg_sw["vol"]
            and _bb_ok(cfg_sw)
        ...
        fast_base = (
            float(adx_1h) >= cfg_f.get("adx", 18)
            and adx_1h_rising
            and vol_ratio >= cfg_f["vol"]
            and _bb_ok(cfg_f)
        )
        ...
        _bb_corridor = (
            cfg_r.get("bb_width_min", 0.8) <= bb_width <= cfg_r.get("bb_width_max", 2.5)
        )
        ranging_base = (
            float(adx_1h) <= _adx_ceil
            and not adx_1h_rising
        ...
        ranging_recovery = (
            _adx_ceil < float(adx_1h) <= 28
            and not adx_1h_rising
            and di_spread_15m >= 12
            and vol_ratio >= cfg_r["vol"]
            and day_position is not None
        )
        _buy_pos_ok = day_position is not None and day_position <= cfg_r.get(
            "buy_pos_max", 0.35
        )
        _sell_pos_ok = day_position is not None and day_position >= cfg_r.get(
            "sell_pos_min", 0.65
        )
        ...
        vwap_side_ok = False
        if vwap:
            if ema_drift_dir == "UP" and close > vwap:
                vwap_side_ok = True
            if ema_drift_dir == "DOWN" and close < vwap:
                vwap_side_ok = True

        drift_vol_ok = vol_ratio >= cfg_d["vol"] and vol_ratio <= BT_DRIFT_MAX_VOL
        delta_opposes = (ema_drift_dir == "UP" and trade_delta_15m < -0.3) or (
            ema_drift_dir == "DOWN" and trade_delta_15m > 0.3
        )
        # Per-pair: require DRIFT direction to align with structural bias_1h
        drift_bias_ok = True
        if pp.get("drift_bias_check", False):
            drift_bias_ok = not (ema_drift_dir == "UP" and bias_1h == "DOWN") and not (
                ema_drift_dir == "DOWN" and bias_1h == "UP"
            )
    ...
    if regime in ("DRIFT", "WEAK_TREND") and atr_15m > 0:
        ...
        if trade_style == "FAST" and side:
            ...
            if (
                trade_style == "FAST"
                and side
                and move_from_base is not None
    ...
    if (
        trade_style != "NO_TRADE"
        and side
        and regime in ("TRENDING", "DRIFT", "WEAK_TREND")
    ):
    ...
    funding_block = False
    if side == "buy" and funding_val > FUND_THRESH:
        funding_block = True
    if side == "sell" and funding_val < -FUND_THRESH:
        funding_block = True
    ...
    if trade_style == "FAST" and side and close:
        ...
        if side == "buy":
            ...
            struct_sl = (
                swings["recent_lows"][-1] - 0.2 * atr_15m
                if swings["recent_lows"]
                else None
            )
            sl_p = round(
                min(struct_sl, atr_sl) if struct_sl and struct_sl < close else atr_sl, 6
            )
            sl_dist = close - sl_p
            ...
            atr_sl = close + atr_sl_dist
            struct_sl = (
                swings["recent_highs"][-1] + 0.2 * atr_15m
                if swings["recent_highs"]
                else None
            )
            sl_p = round(
                max(struct_sl, atr_sl) if struct_sl and struct_sl > close else atr_sl, 6
            )
            sl_dist = sl_p - close
            _tp1_k = BT_DRIFT_TP1_K if regime in ("DRIFT", "WEAK_TREND") else 0.8
            tp_p = round(close - sl_dist * _tp1_k, 6)  # TP1
        ...
        if side == "buy":
            ...
            struct_sl = (
                swings["recent_lows"][-1] - 0.3 * atr_15m
                if swings["recent_lows"]
                else None
            )
            ...
            atr_sl = close + entry_cfg.get("sl_k", 1.8) * 1.2 * atr_15m
            struct_sl = (
                swings["recent_highs"][-1] + 0.3 * atr_15m
                if swings["recent_highs"]
                else None
            )
    ...
    if not vwap_ok:
        _drop = _drop or "vwap"
    if funding_block:
        _drop = _drop or "funding"
    if oi_weak_veto:
        _drop = _drop or "oi_weak"
    if not rr_ok:
        _drop = _drop or "rr_low"
    ...
    out = {
        ...
        "move_from_base": round(move_from_base, 3)
        if move_from_base is not None
        else None,
        "trigger_vol_ratio": round(trigger_vol_ratio, 3)
        if trigger_vol_ratio is not None
        else None,
...
NOTIONAL_RATIO = (
    1.5  # notional = balance × 1.5 (e.g. $1500 at $1000 balance, 10x leverage)
)
...
def check_outcome(
    raw_forward, side, sl, tp, signal_ts_ms, max_hold_ms, entry_price=None
):
    ...
    for c in reversed(raw_forward):
        ...
        if side == "buy":
            if low <= sl:
                return "STOP", mins, sl, round(mfe / sl_dist, 2) if sl_dist > 0 else 0.0
            if high >= tp:
                return "TP", mins, tp, round(mfe / sl_dist, 2) if sl_dist > 0 else 0.0
        else:
            if high >= sl:
                return "STOP", mins, sl, round(mfe / sl_dist, 2) if sl_dist > 0 else 0.0
            if low <= tp:
...
async def run():
    ...
    print(
        f"  Hold:      FAST={HOLD_FAST_M}m  DRIFT_FAST={BT_FAST_HOLD_MIN}m  SWING={HOLD_SWING_M}m"
    )
    print("  TimeBlock: off")
    print("  Position:  one per pair (any side)")
    print("  Fees:      NOT modeled  |  Slippage: NOT modeled")
    print("  Entry:     market at next scan (no fill delay)")
    print("=" * 62)
    print()
    ...
    _all_timestamps = list(
        range(cache_start_ms, now_ms - OUTCOME_H * 3600 * 1000, step_ms)
    )
    ...
    async def _fetch_symbol(symbol: str) -> tuple:
        ...
        print(
            f"  {symbol}: 4H={len(h4)}, 1H={len(h1)}, 15m={len(h15)}, 5m={len(h5)}, "
    ...
    try:
        ...
        cache_valid = (
            ...
            and _tmp.get("_cache_start_ms", cache_start_ms)
            <= cache_start_ms  # covers full range
        )
    except Exception:
        cache_valid = False
    ...
    async def _fetch_mi_history(
        symbol: str, bar: str, endpoint_fn, since_ms: int
    ) -> list:
    ...
    try:
        _mi_tmp = (
            pickle.load(open(CACHE_MI_FILE, "rb")) if CACHE_MI_FILE.exists() else {}
        )
        mi_valid = (
    ...
    if mi_valid:
        ...
        print(
            f"  MI-кэш загружен: {sum(1 for k in mi_cache if k != '_cache_start_ms')} пар"
        )
    else:
    ...
    for pset in PARAM_SETS:
        ...
        def _process_symbol(symbol, ts_list):
            ...
            for ts_ms in ts_list:
                ...
                raw_5m = (
                    list(reversed(h5_all[max(0, i5 - 30) : i5])) if h5_all else None
                )
                raw_mark = (
                    list(reversed(mark15_all[max(0, im - 10) : im]))
                    if mark15_all
                    else []
                )
                raw_idx = (
                    list(reversed(idx15_all[max(0, ii - 10) : ii])) if idx15_all else []
                )
                ...
                if (
                    PERP_DIV_SHORT_VETO is not None
                    and sig["side"] == "sell"
        ...
        for symbol in SYMBOLS:
            sym_res, sym_funnel, sym_ticks, drift_meta = _process_symbol(
                symbol, pset_timestamps
            )
    ...
    # ── Report ─────────────────────────────────────────────────────────────────
    def _report(
        label, results, funnel=None, total_ticks=0, days_back=DAYS_BACK, drift_meta=None
    ):
        ...
        print(
            f"  Winrate (TP+SL):  {winrate}%  ({len(wins)}/{total_r})  [без TIME_EXIT]"
        )
        ...
        if time_x:
            ...
            print("  ⏱ TIME_EXIT диагностика:")
            print(f"    avg mfe_r:  {avg_mfe}R  (пик внутри окна)")
            print(f"    avg exit_r: {avg_exit}R  (итог при выходе)")
            print(
                f"    exit > 0R:  {pos_exit}/{len(time_x)}  ({pos_exit * 100 // len(time_x)}%)"
            )
            print(
                f"    mfe ≥ 0.5R: {half_r}/{len(time_x)}  ({half_r * 100 // len(time_x)}%)"
            )
        ...
        if hour_total:
            hour_wr = {
                h: hour_wins.get(h, 0) * 100 // hour_total[h]
                for h in hour_total
                if hour_total[h] >= 2
            }
            if hour_wr:
                ...
                print(
                    f"  Худший час:  {worst:02d}:00 UTC — {hour_wr[worst]}% ({hour_total[worst]} сигналов)"
                )

        drift_results = [
            r for r in results if r.get("regime") in ("DRIFT", "WEAK_TREND")
        ]
        ...
        drift_pf = (
            round(sum(_r(r) for r in drift_wins) / len(drift_losses), 2)
            if drift_losses
            else 99.0
        )
        drift_wr = (
            len(drift_wins) * 100 // (len(drift_wins) + len(drift_losses))
            if (len(drift_wins) + len(drift_losses)) > 0
            else 0
        )
        drift_avg_r = (
            round(
                sum(r.get("exit_r", 0.0) for r in drift_results) / len(drift_results), 2
            )
            if drift_results
            else 0.0
        )
        drift_blocked = drift_meta.get("blocked", 0)
        drift_candidates = drift_meta.get(
            "candidates", len(drift_results) + drift_blocked
        )
        print(f"\n  ─── DRIFT BREAKDOWN {'─' * 45}")
        print(f"  Total DRIFT signals:   {drift_candidates}")
        print(
            f"  Blocked by new filter: {drift_blocked}  ({(drift_blocked * 100 // drift_candidates) if drift_candidates else 0}%)"
        )
        ...
        for sym in ALL_SYMBOLS:
            ...
            sym_wr = (
                len(sw) * 100 // (len(sw) + len(sl_)) if (len(sw) + len(sl_)) > 0 else 0
            )
        ...
        if BT_DIST_ANALYSIS:
            if any(r.get("vwap_stretch") is not None for r in drift_results):
                winners = [
                    r["vwap_stretch"]
                    for r in drift_results
                    if r["outcome"] == "TP" and r.get("vwap_stretch") is not None
                ]
                losers = [
                    r["vwap_stretch"]
                    for r in drift_results
                ...
                if winners or losers:
                    ...
                    for key in ("p25", "p50", "p75", "p90", "mean"):
                        print(
                            f"  {key}:     {w_map.get(key, 'n/a')}            {l_map.get(key, 'n/a')}"
                        )
            if any(r.get("move_from_base") is not None for r in drift_results):
                winners = [
                    r["move_from_base"]
                    for r in drift_results
                    if r["outcome"] == "TP" and r.get("move_from_base") is not None
                ]
                losers = [
                    r["move_from_base"]
                    for r in drift_results
        ...
        print(
            f"    Сигналов/день ≥2:  {'✅' if signals_per_day >= 2 else '❌'}  {signals_per_day}"
        )
        ...
        print(
            f"  Исполнено:        {executed}  |  Пропущено: {skipped} (позиция занята)"
        )
