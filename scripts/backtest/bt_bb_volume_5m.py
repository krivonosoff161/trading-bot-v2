...
def _bias_1h_series(
    closes_1h: np.ndarray, ema_fast: int = 20, ema_slow: int = 50
) -> np.ndarray:
...
def simulate_symbol(
    sym: str, data: dict, band_to_band: bool = False, vol_mult_override: float = None
) -> list:
    raw_5m = data["5m"]
    raw_1h = data["1h"]
            ...
            if dist >= fp["min_dist"] * cur_atr and slope_fading:
                ...
                if band_to_band:
                    ...
                    tp = (
                        cur_bb_mid
                        if fp["tp_atr"] == 0
                        else cur_close - fp["tp_atr"] * cur_atr
                    )
...
def simulate_symbol_walk(sym: str, data: dict) -> list:
    ...
    for i in range(warmup, n5 - 1):
        ...
        vol_walk_declining = all(
            v5[i - j - 1] >= v5[i - j] for j in range(1, WALK_BARS)
        )
        ...
        if (
            _walked_lower(WALK_BARS)
            and cur_close > cur_bb_lo
            and vol_walk_declining
            and vol_spike
        ):
            sl = cur_bb_lo - fp["sl_atr"] * cur_atr
            tp = cur_bb_up
...
def _stats_line(label: str, trades: list) -> str:
    ...
    return (
        f"  {label:20s}  n={s['n']:4d}  WR={s['wr']:5.1f}%  "
...
def main(cache: dict = None):
        ...
        for p in range(NUM_PERIODS):
            ...
            ts_periods.append(
                (
                    ts_start,
                    ts_end,
                    f"P{p + 1} ({p * DAYS_BACK}–{(p + 1) * DAYS_BACK}d ago)",
                )
            )
    ...
    def _report(label, trades_by_sym, combined_trades):
        ...
        if ts_periods:
            ...
            for ts_start, ts_end, plabel in ts_periods:
                subset = [
                    t for t in combined_trades if ts_start <= t.ts_entry <= ts_end
                ]
    ...
    _report(
        f"Walking the band ({WALK_BARS}+ bars, hold 180m)",
        all_trades_walk,
        combined_walk,
    )
    ...
    print(
        f"  {'BB mid (baseline)':25s}  {s1['n']:>4}  {s1['wr']:>5.1f}%  {s1['pf']:>6.2f}  {s1['avg_r']:>+7.3f}  {s1['time_exit_pct']:>4.0f}%"
    )
    print(
        f"  {'BB mid vol_mult=1.0':25s}  {s4['n']:>4}  {s4['wr']:>5.1f}%  {s4['pf']:>6.2f}  {s4['avg_r']:>+7.3f}  {s4['time_exit_pct']:>4.0f}%"
    )
    print(
        f"  {'Band-to-band (1 touch)':25s}  {s2['n']:>4}  {s2['wr']:>5.1f}%  {s2['pf']:>6.2f}  {s2['avg_r']:>+7.3f}  {s2['time_exit_pct']:>4.0f}%"
    )
    print(
        f"  {f'Walk ({WALK_BARS}+ bars) opp band':25s}  {s3['n']:>4}  {s3['wr']:>5.1f}%  {s3['pf']:>6.2f}  {s3['avg_r']:>+7.3f}  {s3['time_exit_pct']:>4.0f}%"
    )
    ...
    for s, fp in FADE_PARAMS.items():
        print(
            f"    {s}: ADX<{fp['adx_max']}  vol<{fp['vol_mult']}x  SL={fp['sl_atr']}ATR  min_dist={fp['min_dist']}ATR"
        )
...
def optimize_adx(cache: dict, symbols: list, adx_range: list) -> None:
    ...
    for sym in symbols:
        ...
        for adx_val in adx_range:
            ...
            print(
                f"  {adx_val:>6}  {s['n']:>4}  {wr_str:>6}  {pf_str:>6}  {ar_str:>7}{marker}"
            )
...
def optimize_bb_walk(cache: dict) -> None:
    ...
    for period in periods:
        for std in stds:
            ...
            ar_str = f"{s['avg_r']:+.3f}" if s["n"] > 0 else "      -"
            print(
                f"  {period:>6}  {std:>4.1f}  {s['n']:>4}  {wr_str}  {pf_str}  {ar_str}{marker}"
            )
    # restore defaults
    BB_PERIOD = 20
    BB_STD = 2.0
