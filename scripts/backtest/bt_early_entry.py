...
def _simulate_exit(h5_all, h5_ts, entry_ts_ms, entry_price, sl, tp, side, max_hold_ms):
    ...
    for c in fwd:
        ...
        if side == "buy":
            if low <= sl:
                return "STOP", elapsed, sl, -1.0
            if high >= tp:
                return (
                    "TP",
                    elapsed,
                    tp,
                    round(mfe / sl_dist, 2) if sl_dist > 0 else 0.0,
                )
        else:
            if high >= sl:
                return "STOP", elapsed, sl, -1.0
    ...
    exit_r_te = (
        round(dir_mult * (exit_price_te - entry_price) / sl_dist, 2)
        if sl_dist > 0
        else 0.0
    )
...
def main():
        ...
        if side == "buy" and entry_a <= ema_level * 1.001:
            ...
            if pullback_price is None:
                ...
                if sl_dist_b <= 0:
                    ...
                    tp_b = (
                        (entry_b + TP_R * sl_dist_b)
                        if side == "buy"
                        else (entry_b - TP_R * sl_dist_b)
                    )
                    ...
                    if outcome_b == "TP":
                        ...
                        exit_r_b = round(
                            d * (exit_price_b - entry_b) / sl_dist_b_safe, 2
                        )
                    else:
                        exit_r_b = 0.0
    ...
    # ── Print table ────────────────────────────────────────────────────────────
    print(
        f"{'Time':>11} {'Symbol':>10} {'EntryA':>9} {'EMA20':>9} {'EntryB':>9}"
        f" {'Diff%':>6}  {'Out_A':>10} {'R_A':>5}  {'Out_B':>12} {'R_B':>5}"
    )
    print("-" * 100)
    for r in rows:
        print(
            f"{r['ts']:>11} {r['symbol']:>10} {r['entry_a']:>9.4f} {r['ema20']:>9.4f}"
    ...
    def _stats(rows_subset, label):
        ...
        tp = [
            ...
            if r["outcome_b" if "B" in label else "outcome_a"] == "TP"
        ]
        sl = [
            r
            for r in rows_subset
            if r["outcome_b" if "B" in label else "outcome_a"] == "STOP"
        ]
        te = [
            r
            for r in rows_subset
            if r["outcome_b" if "B" in label else "outcome_a"] == "TIME_EXIT"
        ]
        ...
        print(
            f"  {label}: n={len(rows_subset)}  TP={len(tp)} SL={len(sl)} TE={len(te)}"
    ...
    def _stats_generic(rows_s, outcome_key, exit_r_key, label):
        ...
        print(
            f"  {label}: n={len(rows_s)}  TP={len(tp)} SL={len(sl)} TE={len(te)}"
    ...
    rows_b_tradeable = [
        r
        for r in rows
        if r["outcome_b"] not in ("NO_PULLBACK", "NO_IMPROVE", "SKIP", "NO_DATA")
    ]
    _stats_generic(
        rows_b_tradeable, "outcome_b", "exit_r_b", "Метод B (только с откатом к EMA)"
    )
    ...
    if entry_diffs:
        print(
            f"  Entry price improvement: avg={np.mean(entry_diffs):.3f}%  max={max(entry_diffs):.3f}%"
        )
