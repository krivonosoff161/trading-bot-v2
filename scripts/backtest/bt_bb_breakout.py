...
Trade = namedtuple(
    "Trade",
    ["symbol", "side", "ts", "entry", "sl", "tp", "outcome", "elapsed_m", "exit_r"],
)
...
def _atr(h, low, c, period=ATR_PERIOD):
    n = len(c)
    tr = np.zeros(n)
    for i in range(1, n):
        tr[i] = max(
...
def simulate(sym, raw5m, vol_mult, tp_mode, hold_m, rsi_filter=False):
    ...
    for i in range(warmup, n - 1):
        ...
        for j in range(i + 1, n):
            ...
            if side == "buy":
                if low[j] <= sl:
                    outcome, elapsed_m, exit_r = "STOP", elapsed_min, -1.0
                    break
                if h[j] >= tp:
                    outcome, elapsed_m, exit_r = (
                        "TP",
                        elapsed_min,
        ...
        trades.append(
            Trade(sym, side, int(ts[i]), entry, sl, tp, outcome, elapsed_m or 0, exit_r)
        )
...
def main():
    ...
    def _run_sweep(rsi_on):
        ...
        print("  (lower BB + green candle + vol | upper BB + red candle + vol)")
        print(f"{'=' * 70}")
        print(
            f"  {'vol_min':>7}  {'tp':>9}  {'hold':>5}  {'n':>4}  {'WR':>6}  {'PF':>6}  {'avg_R':>7}  {'TIME':>5}"
        )
        print("  " + "-" * 65)
        best = []
            ...
            for tp_mode in tp_modes:
                for hold_m in hold_options:
                    ...
                    for sym, raw in raw5m_by_sym.items():
                        all_trades += simulate(
                            sym, raw, vol_mult, tp_mode, hold_m, rsi_filter=rsi_on
                        )
                    s = _stats(all_trades)
                    if s["n"] == 0:
                        continue
                    ...
                    print(
                        f"  {vol_mult:>7.1f}x  {tp_mode:>9}  {hold_m:>5}m  {s['n']:>4}"
    ...
    print("=" * 70)
    print("  BB FADE now (vol<70%, tp=mid, 60m):  WR=61%  PF=3.71  avg_R=+0.983  n=36")
    print()
    ...
    def _show_top(label, best):
        ...
        print(
            f"  {label}:  vol>={vm}x  tp={tm}  hold={hm}m  "
    ...
    if best_rsi:
        ...
        for sym, raw in raw5m_by_sym.items():
            ...
            print(
                f"  {sym:>12}: n={s['n']:>3}  L={longs:>2} S={shorts:>2}"
