...
| Build forward shadow replay | `python -m scripts.strategy_lab.paper_generation_cutover --private-root <exact-source-root> shadow-replay --shadow-root <new-shadow-root> --code-identity <exact-revision> --validation-generation-id <verified-generation> --now-ms <bounded-time>` | Copies only the authenticated paper-signal ledger into a distinct root, binds PFR producer authority to the explicitly verified validation generation, runs one bounded public-data v2 generation, and reports content parity. It never copies configuration, delivery state, recipients, or credentials. |
...
| Adopt an already durable expired materialization | `python -m scripts.strategy_lab.materialization_recovery plan|apply --private-root <exact-private-root> --task-id <exact-id> ...` | Quiescent, hash-bound recovery for one expired `run_sweep` that already has one exact durable compute binding. It adopts the existing job without requeueing or creating another queue/outbox/materialization effect; drift fails closed and repeat apply changes zero rows. |
| Plan historical Telegram outbox disposition | `python -m scripts.strategy_lab.paper_telegram_outbox_disposition plan --private-root <exact-private-root> --output <private-plan> --json` | Read-only inventory of exact `external_ack_ambiguous` and crash-left `pending` records. The private plan binds the whole outbox, sent-key index, exact records, and private root. |
| Apply permanent Telegram no-replay disposition | `python -m scripts.strategy_lab.paper_telegram_outbox_disposition apply --private-root <exact-private-root> --plan <private-plan> --expected-plan-digest <sha256> --backup <private-backup> --confirm-permanent-no-replay --json` | Requires the canonical delivery claim and exact hash-bound state. Preserves original ACK evidence in an immutable backup and per-record metadata, does not claim completion or add sent keys, and makes the exact content keys permanently non-replayable. Drift fails closed. |
