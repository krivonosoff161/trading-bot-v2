...
if not "%STRATEGY_LAB_LLM_ENABLED%"=="1" (
  echo Strategy Lab overnight LLM loop is a paid-provider path.
  echo Refusing to auto-enable it. Set STRATEGY_LAB_LLM_ENABLED=1 explicitly first.
  echo Safe no-LLM overnight: use python -m scripts.strategy_lab.research_loop --apply --night-mode --duration-minutes 480
...
echo.
python -X utf8 -m scripts.strategy_lab.research_loop --apply --llm-propose --night-mode --load-env .env --duration-minutes 480 --sleep-seconds 60 --max-queued 20 --max-candidates 5 --max-worker-jobs-per-iteration 1 --max-llm-contract-failures 3
echo.
