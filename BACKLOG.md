...
**Отдать GPT бриф** `docs/gpt_pump_restructure_23_05_2026.md` — собрать paper-движок пампа «рывок».
- **Найден рабочий edge (polish 22.05):** «рывок» = ранний ТИКОВЫЙ вход на импульсе волатильных альтов + структурный ride. **+3.03% net, WR 84%, обе стороны, capture ~50%**, BSB +3.68%/EDEN +2.10%. Это памп, сделанный правильно (старый reversal мёртв). Подтверждает ручную торговлю трейдера.
- **⛔ Испыт. срок:** только ~2-3 дня тиков (20-22.05), BSB fat-tail → paper-форвард подтверждает (или нет) на 2-3 недели. **Денег НЕ трогаем.**
- GPT строит `ws_impulse_pump.py` (PAPER, новые файлы, прод не трогать, в start_all НЕ добавлять) + **training-grade записи** (сигнал+исход+valid, формат обучающей БД) + Telegram в памп-группу.
- → **потом АУДИТ Claude** (совпадает ли с research-конфигом, чистые ли записи, прод цел) → запуск → копим дни.

### ⏭️ ПОТОМ — МАЙН (в очереди, НЕ забыт)
Чинить TRENDING/RANGING как **ИНТЕР-ДЕЙ ТРЕНД** (не памп-логикой!). Продовый DRIFT×FAST WR 91% работает — не переизобретать; усилить слабые режимы трендовой логикой (ехать на волне), НЕ импульс+тесный TP.
