...
### 🔴 АКТИВНОЕ — майн: поздний вход
**Ждём GPT** по брифу `docs/gpt_main_entry_timing_21_05_2026.md`.
- Проблема: майн WR 75% (по решённым), НО **41% сигналов выходят по TIME** = вход слишком поздний (движение уже случилось → дрейф вбок → TIME). Подтверждено и цифрами, и визуально трейдером.
- GPT: замер «опоздания» (% хода съедено до входа) + картинки по режимам + альтернативные входы (откат/пробой) + вывод **стиль под каждый режим** (DRIFT/TRENDING/RANGING).
- Метод: графики + честный net, как с continuation. Прод/config НЕ трогаем.
- Принесёт результат → Claude проверит по чеклисту (границы, честность, картинки совпадают с тем что видит трейдер).

### ✅ Решено 21.05
- **Git → PUBLIC** (открыт пользователем; секретов нет, chat ID вынесен в .env).
- **Pump → off** окончательно на этот этап (reversal закрыт fee-blocked). Continuation forward-paper **НЕ запускаем сейчас** — решение пользователя. Думаем как чинить систему.

### 📦 ПАРКОВАНО
- **Continuation** — V3 «не жопа, не вау» (BILL/UB плюс на обе стороны, но тонкие выборки/fat-tail/~10 дней). Модель задокументирована (`continuation_model_v3_20_05_2026.md`). Вернуться если решим копить forward-данные.
- **Обучающая БД — первый кирпич заложен:** `flag_invalid_signals.py` (valid/invalid флаг на main-метках). Остаток: свести main+BB+pump в один формат + определить «удачную сделку» и признаки (нужен ввод пользователя).
- **Накопление S2.3 labeled** — копится пассивно.
- **CVD + oscillation** — для continuation, после валидации базового edge.
