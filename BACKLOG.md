...
## 🧭 Приоритеты на 2026-05-21

> Сверять с этим разделом в начале каждой сессии. Менять при закрытии фаз / появлении блокеров.

### 🔴 АКТИВНОЕ — Phase B: режимы майна
**Ждём GPT** по брифу `docs/gpt_regime_model_phaseB_21_05_2026.md` (с чекпоинтом после классификации).
- **Диагноз (Phase A + разбор картинок трейдером):** майн настроен под DRIFT×FAST (WR 91%) и натянут на все режимы → **TRENDING слишком строгий** (пропускает реальные тренды), **RANGING читает направление неверно**, вход часто **на пике** (EDEN/SOL). Recall на реальных движениях ~0-2%. Сама классификация режима под вопросом.
- **Phase B:** (А) классификация режима = база **[ЧЕКПОИНТ: трейдер подтверждает глазами]** → (Б) per-regime логика → (В) вход на пике → (Г) нужные данные → (Д) единый импульс-детектор main+pump (по волатильности пары).
- **Insight:** пропуски майна и «рвём топ» на пампе — один импульс-паттерн, edge масштабируется волатильностью пары.
- Прод/config НЕ трогаем. Часть А → Claude+трейдер проверяют классификацию, потом Б-Д.

### ✅ Сделано 21.05
- **Git → PUBLIC** (секретов нет, chat ID в .env).
- **Снапшот починен** (`project_snapshot.py` читал старый лог → теперь main_signals + valid-флаг + heartbeat).
- **`flag_invalid_signals.py`** — метит битые метки (precision-bug до 15.05).
- **Phase A (regime coverage)** + main entry-timing — сделаны, лиды найдены (см. диагноз выше).
- **Pump → off**; continuation влит в Phase B как research-компонент (не прод).
