...
**Последнее ревью:** 2026-05-27 — разобрано 12 ТГ-каналов инфополя; автономная охота A(SFP)/B(ротация) — оба OOS-минус; подключён офиц. OKX MCP (данные); sentiment-зонд дал слабый ПЛЮС (намёк, не значимость) → кандидат на форвард-сбор
