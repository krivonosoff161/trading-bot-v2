...
### 🔔 НА ЗАВТРА (21.05) — первый шаг
**Решить: запускаем forward-paper фиксированной continuation-модели?**
- V3 сделан (коммит `86efe34`). Вердикт: **«не жопа, не вау» — осторожно-перспективно.** Фиксированная модель (кластер 2+/5м + ранний вход 0.3%≤10с + фильтр силы свечи 1.5x + стоп за импульс + структурный выход) даёт **плюс на ОБЕ стороны на нескольких парах**: BILL +1.09% (n=59, стабильна рано/поздно), UB +1.28% (n=32, стабильна), EDEN/TRUTH/BSB тоже плюс но односторонние по времени. Всё net после 0.20% тейкер.
- **Но:** тонкие под-выборки (BILL late-long n=7), fat-tail (медиана<среднего), всего ~10 дней. Подтвердить нельзя → **в config/прод НЕ идём.**
- **Следующий шаг:** заморозить параметры модели (НЕ подгонять!) и запустить **forward-paper наблюдатель** (без денег) на BILL/UB/EDEN/TRUTH/BSB → копить новые дни → через 2-3 недели проверить держится ли BILL/UB и добирают ли остальные вторую сторону.
- Если решаем «да» → написать GPT бриф на сборку наблюдателя. Это ОДНА проверялка рядом с майном, не возврат к зоопарку движков.
- Модель задокументирована: `docs/...` / `scripts/analysis/research/output/continuation_model_v3_20_05_2026.md`.
