...
**Последнее ревью:** 2026-05-23 (памп реструктуризирован в paper-движок «рывок» + закрыт → следующий фокус МАЙН)
...
### 🔔 ПЕРВЫМ ДЕЛОМ (новая сессия) — МАЙН ДВИЖОК
Чинить TRENDING/RANGING как **ИНТЕР-ДЕЙ ТРЕНД** (не памп-логикой!). Продовый DRIFT×FAST WR 91% работает —
**не переизобретать** (GPT-реимпл WR ~20%, направление сломано); усилить слабые режимы **трендовой логикой**
(ехать на волне), НЕ импульс+тесный TP. Диагноз режимов готов (21.05): TRENDING фильтры слишком строги
(пропускает реальные тренды), RANGING читает направление неверно, вход часто на пике. Эталон = DRIFT×FAST,
период как Phase A, GO/NO-GO: net+ после fee, обе стороны, держится во времени, n≥20. Прод не трогать,
всё в research-харнессе. При проверке кейсов — **открывать ВСЕ картинки сразу** (косяк 21.05).
