...
## 🧭 Приоритеты на 2026-05-22

> Сверять с этим разделом в начале каждой сессии. Менять при закрытии фаз / появлении блокеров.

### 🔔 ПЕРВЫМ ДЕЛОМ (новая сессия)
**Отправить GPT бриф ре-рана** `docs/gpt_regime_exit_rerun_22_05_2026.md` — меняем ТОЛЬКО выход.
- **Корень NO-GO найден трейдером по картинкам:** выход сломан. TP = фикс `1.1R/1.4R` ≈ **1.4 ATR** — крошечный, НЕ связан с размером движения. На альте +14% забирает 1-2% и выходит → комса доедает огрызок. Вход при этом ЧАСТО ХОРОШИЙ; душит TP + берётся не тот (мелкий) кусок, большой импульс проспали.
- **Ре-ран:** классификацию/вход/направление/peak-guard НЕ трогать → поменять ТОЛЬКО выход на «езжай на движении» (структурный трейл как `continuation_model_v3` / giveback) + **структурный стоп** (за импульсом, не тесный ATR). Показать дельту net по ячейкам vs старый фикс-TP.
- **Ожидание (не подгонять):** волатильные альты улучшатся, майоры скорее fee-blocked — проверить цифрами.
- Принёс GPT → Claude проверяет + **открывает ВСЕ кейсы сразу** (21.05 косяк — дал вердикт по цифрам без картинок).

### ✅ Сделано 21.05
- **Git → PUBLIC** (секретов нет, chat ID в .env). **Снапшот починен** (читал старый лог → теперь main_signals + valid + heartbeat). **`flag_invalid_signals.py`** метит битые метки.
- **Phase A** (regime coverage) — recall ~0-2%, корень молчания = фильтры подтверждения.
- **Phase B Часть А** — классификация исправлена (старый RANGING был в 9× раздут; свинги → TRENDING), трейдер подтвердил глазами. **Чекпоинт пройден.**
- **Phase B Б-Д** — per-regime модели NO-GO, НО корень = тесный фикс-TP (см. «первым делом»). Шорт-импульсы на волатильных альтах = единственная живая нить.
- **Pump → off**; continuation влит в импульс-исследование (не прод).

### 📦 ПАРКОВАНО
- **Continuation V3** — «не жопа, не вау» (BILL/UB плюс на обе стороны, но тонко/fat-tail/~10 дней). Модель `continuation_model_v3_20_05_2026.md`. Сейчас живёт как импульс-компонент Phase B (Часть Д).
- **Обучающая БД** — первый кирпич `flag_invalid_signals.py` (71 valid / 19 invalid). Остаток: свести main+BB+pump в один формат + определить «удачную сделку» и признаки (ввод пользователя).
