...
## 🔔 НАПОМИНАНИЯ (ближайшее)

- **🟢 31.05 — НОВОЕ НАПРАВЛЕНИЕ = ИНФО-ЭДЖ СКАНЕР (форвард-логгер). Пункты:**
  - **▶ Следующий ШАГ (не бэклог, ждёт GO+дизайн): форвард paper-логгер** — event collector (листинги/vol-shock/funding-OI/новости) → пре-фильтр → LLM-решение → paper-исполнение → табло vs baseline → JSONL. На полке scout+analyzer+OKX. Деньги только после форвард-доказательства net+.
  - **⏳ Локальная LLM для decision-слоя** — когда сканер оправдается. ПК слабый (GTX1050 3GB / 8GB RAM) → реалистично 3-7B квантиз. (Ollama/LM Studio); AirLLM-большая = не для realtime (послойная загрузка медленная). Decision-слой плаг-ин (Яндекс↔локаль), A/B на одном paper-логе.
  - **⏳ 2 WATCH-кандидата** (не торгуемы, мониторить форвардом): `xsec_mom_topQ_look14` (p=0.051); `btc_alt_lag1_4H` (4H-only, регим-артефакт).
  - **⏳ risk-off флаг** `supply_contraction`→BTC («инфо не деньги», perm-p<0.05 робастно 8/9 лет) → кормить КОНТЕКСТ анализатора, не трейд.
  - **⏳ Платные on-chain данные** (CryptoQuant/Glassnode) — реальный flow→price lead за платными ключами (scout2 30.05); решение трейдера.
  - **⏳ ScoutDaily в планировщик** (рука трейдера) → накопитель `forward_series` сам копит OOS-данные.
- **🔴 29.05 — ВХОД-ТАЙМИНГ ПРОТЕСТИРОВАН OOS → МЁРТВ как рычаг прибыльности.** Гипотеза «оттянуть вход раньше
  починит сетап» НЕ выжила на апрельском OOS (все 3 модели причинности: ранний вход ХУЖЕ позднего; объединённый
  двусторонний ещё хуже). Аудит поймал look-ahead в первом прогоне (как 26.05). **Под реалистичным филлом весь
  движок net-минус (~−58R апрель)** = тайминг не рычаг, констрейнт = тонкий/sub-cost эдж (3-е подтверждение
  28.05). НЕ строить ранний движок. Канон `research/docs/anticipation_oos_results_2026-05-29.md`.
- **⏳ drift_short_veto — пересмотреть ПРЕМИСУ (не снимать сейчас).** OOS-апрель: вето-шорты с поздним входом
  net-плюс (+0.035R/трейд, направление ок) → премиса вето «не та сторона» НЕВЕРНА. НО sub-cost-хрупко, под
  реалистичным филлом минус. Нужен 2-й месяц OOS + чинёный харнесс ДО любого live-изменения. Снятие вето = вне
  10-дневной заmorozки (config/engine). Не действовать.
- **⏳ Починить OOS-харнесс** (`anticipation_oos_harness_2026-05-29.py`): скан выхода с d+1; причинный 1h/4h;
  репортить fill-полосу (close[d-1] репорт vs close[d] реальный). + починить OI-фид (oi_delta мёртв — пустой).
- **⏳ 10-ДНЕВНЫЙ ЦИКЛ (старт 28.05.2026)** — бот запущен с пакетом правок (см. ниже). Не трогаем 10 дней.
- **⏳ Cycle-2 правки (после наблюдения 10 дней):**
  - **Pair-filter для катастрофических SL пар** (BSB/EDEN/MEW/BOME/USELESS-типа) — отложено, требует понимания как `ws_main_screener.py` выбирает юниверс. Сначала смотрим что бот реально ловит на top_60.
  - **SWING trailing-ATR k=1.0** — Test A (28.05) нашёл Δ +0.86%/трейд на n=8 ex-outliers. Требует динамической логики в engine + взаимодействия с OKX API (обновление SL по MFE). Сначала за 10 дней соберём больше SWING-сигналов на чистом baseline.
  - **🆕 Entry-price vs real fill (методология #2).** Обнаружено 28.05 при live-проверке: `signal_snapshot.entry` = close какой-то старшей свечи (15m или 1h), на которой движок решал. Реальный market-fill уезжает от этого числа на 0.2-0.3% адверс-slip (видно на ENA-USDT-SWAP @ 14:45 UTC: записано 0.08696, реальная цена в момент сигнала 0.08678). Лейблер market-entry (фикс 6a46188) использует bar-цены после сигнала — выходы считаются корректно. НО `exit_r` в журнале считается от `entry` → может слегка завышать достигнутый R. Меньше первой методологической дыры, но похожая природа. Разобраться на 10-дневном датасете: где именно проставляется `entry` в `signal_engine.py`, какой ТФ берётся, насколько адверс-slip систематичен (мажоры vs альты, FAST vs SWING). Решить: фиксить запись entry на «next-bar open» (true fill price) или просто учитывать в R-multiple расчёте.
- **⏳ ~28.05 — ЗАПУСК нового прогона на чистом `logs/`** + наблюдение «что и как». Трейдер запускает сам. После — снять, как пишутся свежие данные, журнал, ТГ.
- **⏳ Публичный devlog-тизер.** Вести в публичном репо журнал общими мазками (что/как делалось по подходу), **невзначай** указывая, что детальная проверка гипотез / анализ / research — в приватном репо. Цель: витрина для редкого живого посетителя (трафик: 237 «клонеров» = боты-скрейперы, живых 1 — он и есть цель), бот хавает пустышку, человек видит закрытый моат. Держать ЧЕСТНО — описываем активность/подход, без ложных заявок о доходности (не инфоцыганство).

---

## 🕸️ 07.06 — L6 «макро/мир» + движок связей (Gotham-ядро) — ПАРКОВАНО (решение трейдера)

**Контекст:** при сборке 5-слойного сканера трейдер указал, что система слепа к событиям мирового масштаба (пример: «Китай начал операцию у Тайваня» — сканер не увидел вообще). Разобрали глубоко → концепция выросла до **Palantir Gotham** (граф сущностей + распространение сигнала + оркестрация внимания). Решение трейдера: **в бэклог, вернуться позже.** Красная линия удержана.

**Суть (L6 = горизонталь, не ещё одна вертикаль):**
- L1–L5 — вертикали: одно событие → один актив. **L6** — одно мировое событие → **каскад по всем слоям** со стороной на каждый актив (Тайвань → TSMC↓, нефть↑, золото↑, BTC↓, оборонка↑, USD↑).
- **Движок связей:** каждый агент тащит не только свой актив, а **`linked_entities + next_watch`**. Пример трейдера: новость о дыре в ZEC содержала в ТЕЛЕ, что аудитор добавил **XMR** в очередь на такой же аудит → LEADING-сигнал на XMR, которого на рынке ещё нет. Эдж — во ВТОРОМ звене, которое почти никто не связал.
- Эти watch'и = **inferred pending events** → наполняют `pending_events` НЕ из календаря, а из вывода цепочки → реализация → **surprise**. Это и оживляет сюрприз-движок (Codex: «pending пустой»).
- **L6 = координатор топологии + директор внимания:** держит связи (Тайвань→TSMC, цепочки поставок, классы privacy-coin), направляет слои **точечно** («L2, следи за XMR, событие аудит, обе стороны»). Watch поднимает ВНИМАНИЕ, не ВЕРДИКТ — агенты судят независимо (анти-стадо).

**Инсайты (чтоб не строить лишнего):**
1. **Эдж = глубокое ЧТЕНИЕ закопанных forward-фактов, не макро-пророчество.** Факт про XMR лежал в теле; рынок проскочил (читали заголовок). Модель-выведенные каскады (Тайвань→TSMC) в основном очевидны/в цене + галлюцинируют.
2. **Дёшево (костыль трейдера соблюсти):** детерм. фильтр (0 ток) → **дешёвый qwen3-30b читает ПОЛНУЮ чистую доку** + тащит facts/links (копейки) → **chief видит ТОЛЬКО компактную выжимку**. ВАЖНО: сжатие — на вход chief, НЕ перед извлечением (саммари выкинет закопанный XMR-факт = убьёт эдж).
3. **НЕ строим граф знаний** — топологию знает сильная модель; наша работа = заземлить её вывод на наши тикеры + watch.

**Почему ПАРКОВАНО (красная линия — это убивало проект):**
- **Время непознаваемо** (watch XMR — когда выстрелит?) · **сторона непознаваема до события** (Трамп: удар по проливу ИЛИ покупка тайваньской компании = противоположные стороны от одного актора) · **полный каскад = Gotham-масштаб** (Palantir = 4000 инженеров; у нас — трейдер + capped-ключ Alibaba + GTX1050).
- Предсказание времени/стороны/каскада = быть оракулом = ровно decision-движок, который **уже выкинули 30.05** (надо analysis/рекомендация, решение человеку).

**Дисциплинированное ЯДРО (если вернёмся — только это):**
> подготовленное внимание + быстрая ФАКТИЧЕСКАЯ реакция + человек решает + всё в датасет.
На макро-шоке: распознать режим-класс → выложить физические факты экспозиции (TSMC на Тайване) → **алерт человеку**, НЕ «вот сделка». Watch направляет внимание, не торгует.

**Когда вернуться:** после того как 5 слоёв + forward-резолвер дадут ЦИФРЫ, что связи реально платят (не мираж). Дешёвое семя (`linked_entities + next_watch` + запись в pending) можно добавить раньше — лёгкое — но мерить честно с первого дня.

---

## 🌙 28.05 НОЧЬ — аудиты (execution + корпус): что пере-прогнать и починить

Два мульти-агентных аудита того же класса, что нашёл дыру лейблера. Каноны (с цифрами):
`research/docs/{execution_audit,corpus_methodology_audit}_2026-05-28.md`.

**Пере-прогнать (возможно зря похоронили эдж — по убыванию ценности):**
- **⭐ polarity-flip без черри-пик-обрезки** — объявлен мёртвым, сырые данные говорят обратное; на малой
  выборке честный вердикт «не определено, мало n». Дёшево, безопасно, бота не трогает. **Первым делом завтра.**
- **«Бот слеп к крупным ходам» (`universe_vs_signals.py`)** — пере-прогнать с честным знаменателем (юниверс
  бота, не oracle на всех парах). Вывод «отбор бесполезен» не доказан, мог двигать решения (24→60).
- (по желанию, позже) cross-sectional с point-in-time вселенной (с делистами); тренд v2/v3 с turnover-костом;
  positioning как форвард-сбор. Все три — смещение ПРОТИВ стратегии, могли занизить.

**Пакет фиксов лейблера/журнала (измерение, не прод-движок — можно во время заморозки):**
- учесть торговые издержки в R-пути (из config, показывать gross И net рядом);
- TIME-сделки сделать реальным исходом (не выбрасывать из метрик);
- скорить цель, которую реально ставит прод (single-TP), не более лёгкую;
- начинать скоринг с первого живого бара (сейчас первый участок окна пропускается);
- гард на дегенеративные строки (копеечные монеты, округление);
- поправить ложный комментарий про учёт издержек.
→ затем пере-релейбл архива + пере-прогон тестов на честных net-числах.

**Поправить устаревший текст:** geometry «V3 в плюс» (уже отозвано — фантом нефилл-сделок); polarity-flip
«мёртв» → «не определено». (Память/PLAN/SESSION уже поправлены 28.05 ночью.)

**Метод-долг (на будущее, чтобы не повторять баг-класс):** единая таблица издержек из config (не 3 разных
в скриптах); turnover-привязанный кост; point-in-time вселенная с делистами; block-bootstrap/Newey-West t
вместо наивного (перекрытие раздувает значимость).

**Инфра сделано (28.05 ночь):** money-guard хук починен (абсолютный путь) + расширен на PowerShell; выданы
права Agent/Workflow/WebFetch для автономного research (деньги/прод/.env/AUTO_TRADE/config под защитой).
Правка хука требует `/hooks` reload или рестарта.

---

## 🔬 Research 27.05.2026 — 12 каналов + охота A/B + OKX MCP + sentiment

Доки: `docs/{telegram_channels_synthesis,branch_ab_results,okx_mcp_sentiment}_2026-05-27.md`. Дисциплина: фикс-предсказание + holdout OOS.

**Что протестировано и как (всё ❌ кроме sentiment-намёка):**
- 🧪 **A. SFP / liquidity-sweep** (`sfp_sweep_test.py` тонкие альты 5m, `sfp_majors.py` мажоры 15m, train/holdout, косты): альты net −0.12%; мажоры gross≈0 (BTC лучший +0.019% gross < косты), net минус. = sub-cost сигнал, ритейлом не достать. **Закрыто.**
- 🧪 **B. ETHBTC / ротация доминаций** (`branch_b_rotation.py`, 2-летние дневки OKX, `okx_history.py`): train Sharpe +1.65/+125% → **holdout −2.7**; ETHBTC IC в holdout −0.12 (знак обратный канальной заявке). Train-мираж → OOS-смерть. **Закрыто.**
- ⏳ **Sentiment (OKX news)** (`sentiment_edge_peek.py`, ~30дн×15монет): IC(net_sentiment→ход 1д) +0.095, t=1.9, спред +0.88% — слабо-ПОЛОЖИТЕЛЬНО, но t завышен перекрытием (~29 независ. дней, 1 режим, без OOS). **Намёк, не доказательство → см. идею форвард-сбора ниже.**

### ⏳ Sentiment forward-collection (кандидат, ждёт GO трейдера)
Глубокого исторического бэктеста sentiment нет (OKX отдаёт ~30 точек trend). Идея: **лёгкий ежедневный read-only сборщик**
(net_sentiment + mentionCnt + цена по корзине монет → дописывать в CSV), копить **чистый неперекрытый** ряд 3-4 недели,
затем честный OOS-тест sentiment→forward-ход. Это «накопить данные, которых нет в истории» — единственный кандидат охоты,
переживший зонд. НЕ бот, НЕ автоторговля — только сбор данных. Источник: офиц. OKX MCP (`okx-official`, read-only).

### Branch C — не тестировано (нужен капитал)
Фарм эйрдропов + дельта-нейтральный арбитраж перп-DEX (из канала Market Players) — не предсказание направления, а операционная активность.
...
### 🟡 26.05 — ТАЙМИНГ ВХОДА исследован: относит. улучшение робастно, абсолют НЕТ → чиним САМ сигнал
Прошли развилку 24.05 «новым путём» = тайминг входа (коммит `bb7507d`, `scripts/analysis/research/entry_*.py`).
**Диагноз:** направление ВЕРНОЕ (96% сигналов имели ход ≥1%), но бот входит **поздно** (на всплеске объёма;
vol-калибровка перевёрнута, vol>3.4=дуды). Урок метода: di_dir/bb_expanding — тени adx (r=0.79), видно только
мультивариантом. **Правило** (вход на развороте отката, не на пробое) на тик-тейпе майя: +0.34% vs наш −0.27%,
DRIFT +1.01%, робастно по 9 геометриям — казалось прорыв. **Но архив-OOS (апрель, 170 сигналов, 102 DRIFT,
5m OKX) НЕ подтвердил:** DRIFT +1.01%→−0.12%, всё минус → майский плюс **период-специфичен**.
**Робастно ТОЛЬКО относительное:** правило системно МЕНЕЕ плохое, чем наш вход (≠ прибыль). **Движок рано.**
**Ложный прорыв пойман ДО сборки** (урок импульса). Тейп = стена 11.05.
**🔀 РЕШЕНИЕ:** не костыль поверх входа, а **чинить момент САМОГО сигнала** (почему скринер опаздывает).
→ research-задача GPT: `docs/gpt_task_early_entry_26_05_2026.md`. Этот пункт перекрывает «Late Momentum Entry
Filter» (P1) — теперь он не «фильтр сверху», а пересмотр триггера скринера.

### 🛑 24.05 ВЕЧЕР — СТОП-ПОЙНТ [ЗАКРЫТО → см. 26.05 выше]
