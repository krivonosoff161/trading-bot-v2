...
├─ Время: закрыть через 6ч (СВИНГ) / 2ч (СКАЛЬП) если уровни не достигнуты
├─ Размер: 2-3% депозита на сделку
├─ 📊 График: торгуй на 15m — не смотри 1m, там шум
├─ ⚡ Плечо выше 5x = стоп выбивается в 3-4 раза чаще, если не менять параметры SL
...
❌ НЕ ДЕЛАТЬ
[1-2 конкретных запрета — ТОЛЬКО про эту ситуацию: нет тренда = нет входа.
ЗАПРЕЩЕНО писать общие советы вроде "не торгуй краткосрочно" или "не гонись за движением" — это противоречит сервису. Пиши конкретно: "не входить в LONG/SHORT без подтверждения тренда на 1H", "не интерпретировать движение на 5m без контекста старшего таймфрейма"]
