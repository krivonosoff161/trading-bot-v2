...
_MODEL_URI       = "gpt://b1git4svubpojuiga5pn/yandexgpt/latest"  # qwen3-235b down, temp fallback
...
ЗАПРЕЩЕНО:
- ADX, DI, EMA, ATR, ratio, перцентиль, SuperTrend, Bollinger, BB — клиент не знает что это
...
- "волатильность низкая, рынок сжался" вместо "ATR перцентиль 2"
...
_STATUS_LABELS = {"ENTRY": "ВХОД", "WAIT": "НАБЛЮДАЕМ", "NO_TRADE": "ВНЕ РЫНКА"}
...
def _build_analysis_text(symbol: str, captured_at: str, snapshot: dict) -> str:
    ...
    mode_cmd = {"ENTRY": "РЕЖИМ 1 — ВХОД", "WAIT": "РЕЖИМ 2 — ЖДЁМ", "NO_TRADE": "РЕЖИМ 3 — НЕТ ТОРГОВЛИ"}.get(entry_signal, "РЕЖИМ 2 — ЖДЁМ")
    lines = [
        f"РЕЖИМ: {mode_cmd}",
