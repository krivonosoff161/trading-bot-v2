...
def run_cycle(
    ...
    *,
    ...
    def _universe() -> dict[str, Any]:
        ...
        applied = (
            _apply(private_root, res["intake_events"], now=now)
            if apply_intake
            else {"registered": 0}
        )
        return {
            "tickers": res["tickers_seen"],
            "intake_events": len(res["intake_events"]),
    ...
    def _validate() -> dict[str, Any]:
        ...
        return {
            "cells": rep["evaluated_cells"],
            "by_cell": rep["summary"]["by_cell"][:6],
        }

    def _tactical() -> dict[str, Any]:
        from src.research_lab.tactical_track import (
            build_track,
            summarize,
            write_snapshot,
        )
        ...
        return {
            k: summarize(build_track(private_root))[k]
            for k in (
                "tactical_leads",
    ...
    def _exit() -> dict[str, Any]:
        ...
        return {
            k: rep["summary"][k]
            for k in ("evaluated", "exit_recovered_candidate", "needs_forward_only")
        }
    ...
    def _memory() -> dict[str, Any]:
        ...
        return {
            "total": s["total"],
            "one_shot": s.get("one_shot_candidates"),
            "cost_bound_maker_unlock": s.get("cost_bound_maker_unlock"),
        }

    for name, fn in (
        ("live_universe", _universe),
        ("mover_validation", _validate),
        ("tactical_track", _tactical),
        ("exit_first_resim", _exit),
        ("outcome_memory", _memory),
    ):
    ...
    report = {
        ...
        "steps": steps,
        "what_worked_failed": _synthesize(steps),
    ...
    for cell in val.get("by_cell", []):
        ...
        if cell["verdict"] == "holds_oos_candidate":
            ...
            failed.append(
                {
                    "what": line,
                    "why": "in-sample positive collapses out-of-sample (overfit)",
            ...
            failed.append(
                {
                    "what": line,
                    "why": "flat/negative OOS on the mover universe",
    ...
    for s in steps:
        if str(s.get("status", "")).startswith("error"):
            failed.append(
                {
                    "what": str(s["step"]),
                    "why": str(s.get("status") or ""),
                    "next": "inspect detail; re-run step",
                }
            )
    return {
        "worked": worked,
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Bounded continuous discovery cycle (research-only)."
    )
    ap.add_argument(
        "--private-root",
    ...
    ap.add_argument(
        "--no-apply", action="store_true", help="do not register intake events"
    )
    args = ap.parse_args()
    rep = run_cycle(
        Path(args.private_root),
        limit_symbols=args.limit_symbols,
        apply_intake=not args.no_apply,
    )
    print(
        json.dumps(
            {
                "steps": [
                    {"step": s["step"], "status": s["status"]} for s in rep["steps"]
                ],
                "what_worked_failed": rep["what_worked_failed"],
            },
            ensure_ascii=False,
            indent=2,
        )
