...
def migrate_json_candles(
    private_root: str | Path, *, apply: bool = False, target_schema: str = "v2",
    migration_available_at_ms: int | None = None,
) -> dict[str, Any]:
    if str(target_schema).lower() != "v2":
        raise ValueError("only synthetic Candle Store v2 migration reports are supported")
    root = resolve_private_root(private_root)
    ...
    migration_time = int(
        ...
        if migration_available_at_ms is not None else time.time() * 1000
    )
    ...
    for path, timeframe in discover_json_candles(root):
        ...
        try:
            ...
            files.append(MigrationFile(
                path=relative_path, symbol="", timeframe=timeframe,
                rows=0, first_ts=None, last_ts=None, status="failed",
        ...
        if apply:
            try:
                ...
                if (
                    ...
                    and candle_slice_fingerprint(symbol, timeframe, existing) == source_fingerprint
                ):
