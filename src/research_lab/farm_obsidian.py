...
def _daily_note(day: str, rows: list[dict[str, Any]]) -> str:
    ...
    top = sorted(
        (r for r in rows if r["validation_status"] in _REGISTRY_STATUSES),
        key=lambda r: float(r.get("avg_net_pct") or 0.0),
        reverse=True,
    )[:10]
    ...
    lines = [
        f"# Farm Run {day}",
        "",
        "Daily research memory. Labels only, not a profitability claim.",
        "",
    ...
    for r in top:
        lines.append(
            f"- [[Symbols/{_safe(r['symbol'])}]] -> [[Families/{_safe(r['family'])}]] "
    ...
    lines += [
        ...
        "## Next",
        "",
        "- review FORWARD_PAPER/REGIME_SPECIFIC via export_hard_validation_requests",
...
def _family_note(family: str, rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"- required data: {', '.join(required) or 'OHLCV only'}",
        f"- GPU signal kernel: {gpu}",
        f"- known failure modes: {risk}",
        f"- data tags: {', '.join(tags) or '-'}",
...
def write_farm_notes(
    private_root: Path, *, day: str, allow_public_output: bool = False
) -> dict[str, Any]:
    ...
    (base / "Runs" / f"farm_{_safe(day)}.md").write_text(
        _daily_note(day, rows), encoding="utf-8"
    )
    counts["runs"] = 1
    for symbol in sorted({r["symbol"] for r in rows}):
        (base / "Symbols" / f"{_safe(symbol)}.md").write_text(
            _symbol_note(symbol, rows), encoding="utf-8"
        )
        counts["symbols"] += 1
    for family in sorted({r["family"] for r in rows}):
        (base / "Families" / f"{_safe(family)}.md").write_text(
            _family_note(family, rows), encoding="utf-8"
        )
        counts["families"] += 1
    ...
    for row in rows:
        ...
        (base / "Candidates" / fname).write_text(
            _candidate_note(row, registry.get(cid, {})), encoding="utf-8"
        )
        counts["candidates"] += 1
