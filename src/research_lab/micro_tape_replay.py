...
TAKER_ROUNDTRIP_PCT = (
    0.10  # conservative fast-scalp cost (taker both sides), percent points
)
...
def _events(
    trades: list[tuple[int, str, float, float]], *, max_events: int
) -> list[dict[str, Any]]:
    ...
    for i, (ts, _side, price, _sz) in enumerate(trades):
        ...
        while trades[lo][0] < ts - LOOKBACK_MS:
            lo += 1
        window = [
            {"ts_ms": t, "side": s, "size": z} for (t, s, _p, z) in trades[lo : i + 1]
        ]
        d = tape_delta(window)
...
def _follow_through(
    trades: list[tuple[int, str, float, float]], ev: dict[str, Any]
) -> dict[str, Any]:
    ...
    target_idx: dict[int, float | None] = {h: None for h in HORIZONS_MS}
    hi = lo = entry
    for ts, _s, price, _z in trades[ev["idx"] + 1 :]:
        ...
        for h in HORIZONS_MS:
            if target_idx[h] is None and dt >= h:
                ...
                target_idx[h] = price
    mfe = (
        dirn * ((hi if not short else entry) / entry - 1) * 100
        if not short
        else (entry - lo) / entry * 100
    )
    mae = (entry - lo) / entry * 100 if not short else (hi - entry) / entry * 100
    prim = horizons.get(f"{PRIMARY_HORIZON_MS // 1000}s")
    return {
        "horizons_net_pct": horizons,
        "primary_net_pct": prim,
        "mfe_pct": round(max(0.0, mfe), 4),
        "mae_pct": round(max(0.0, mae), 4),
    }
...
def _manifest_files(
    private_root: Path, *, max_files: int, symbols: list[str] | None
) -> list[tuple[str, str]]:
...
def run(
    ...
    *,
    ...
    return {
        ...
        "files_replayed": files_done,
        "events": len(all_rows),
        "summary": _summarize(all_rows, per_symbol),
    }
...
def _horizon_curve(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for h in HORIZONS_MS:
        key = f"{h // 1000}s"
        nets = [
            r["horizons_net_pct"][key]
            for r in rows
            if key in (r.get("horizons_net_pct") or {})
        ]
        if nets:
            out[key] = {
...
def _summarize(
    rows: list[dict[str, Any]], per_symbol: dict[str, list[dict[str, Any]]]
) -> dict[str, Any]:
    scored = [r for r in rows if r.get("primary_net_pct") is not None]
    ...
    for sd in ("short", "long"):
        ...
        by_side[sd] = {
            "n": len(s),
            "median_net_pct": round(median(nets), 4) if nets else 0.0,
            "win_rate": round(sum(1 for x in nets if x > 0) / len(nets), 4)
            if nets
            else 0.0,
            "bucket": _bucket(s),
            "horizon_curve": _horizon_curve(s),
        }
    return {
        "events_scored": len(scored),
        "primary_horizon": f"{PRIMARY_HORIZON_MS // 1000}s",
        "by_side": by_side,
        "overall_bucket": _bucket(scored),
...
def write_snapshot(private_root: Path, report: dict[str, Any]) -> Path:
    ...
    payload = {
        ...
        "files_available": report.get("files_available"),
        "files_replayed": report.get("files_replayed"),
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Tape-pressure follow-through replay on the real tape (research-only)."
    )
    ap.add_argument(
        "--private-root",
    ...
    ap.add_argument(
        "--symbols", default="", help="comma-separated instIds to restrict to"
    )
    ...
    print(
        json.dumps(
            {
                k: report[k]
                for k in ("files_available", "files_replayed", "events", "summary")
            },
            ensure_ascii=False,
            indent=2,
        )
