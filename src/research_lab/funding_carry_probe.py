...
def run(private_root: Path, *, limit_symbols: int = 40, provider: Any = None) -> dict[str, Any]:
    ...
    for sym in symbols:
        ...
        if m is None:
            continue
        per_symbol[sym] = m
        all_eps.extend(_episodes(sorted(pts)))
    return {"symbols": len(per_symbol), "total_episodes": len(all_eps),
            "summary": _summarize(per_symbol, all_eps), "per_symbol": per_symbol}


def _summarize(per_symbol: dict[str, dict[str, Any]], all_eps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    med_net = median(nets)
    ranked = sorted(per_symbol.items(),
                    key=lambda kv: -(kv[1].get("annualized_carry_pct") or 0))
    verdict = ("carry_beats_cost_candidate" if med_net > 0 and pos_share >= 0.55
               else "carry_below_cost" if med_net <= 0 else "marginal")
    return {"pooled_episodes": len(all_eps), "median_episode_net_pct": round(med_net * 100, 4),
            "episode_net_positive_share": round(pos_share, 3),
            "top_carry_symbols": [{"symbol": s, "annualized_carry_pct": m.get("annualized_carry_pct"),
                                   "episode_net_positive_share": m.get("episode_net_positive_share")}
                                  for s, m in ranked[:10]],
            "verdict": verdict,
            "note": "delta-neutral funding harvest, net of a 0.20% two-leg round-trip. Gross-of-basis-risk; "
                    "carry != edge until basis/execution risk is tested. Nothing paper-ready"}
