...
def _exit_modes(params: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    return [
        ...
        (
            ...
            {
                "kind": "partial",
                "tp1_pct": round(take * 0.5, 4),
                "tp2_pct": round(take, 4),
            },
        ),
    ...
    for j in range(idx, cap + 1):
        ...
        if kind == "trailing":
            ...
            if trail is not None:
                stop = (
                    max(init_stop or trail, trail)
                    if long_
                    else min(init_stop or trail, trail)
                )
        elif kind == "break_even":
            be = _levels(entry, mode["be_trigger_pct"], long_=long_)
            if be is not None and (
                (long_ and water >= be) or (not long_ and water <= be)
            ):
                stop = entry
...
def _partial_net(
    ...
    for j in range(idx, cap + 1):
        ...
        if not filled1:
            if stop is not None and (
                (long_ and lo <= stop) or (not long_ and hi >= stop)
            ):
                return _mk_partial(
                    candles, idx, j, side, entry, stop, stop, 0.0, cost_pct, "stop"
                )
            if tp1 is not None and ((long_ and hi >= tp1) or (not long_ and lo <= tp1)):
                filled1, stop = (
                    True,
                    entry,
                )  # half off, rest to break-even; rest trades from NEXT bar
                continue
            ...
            if tp2 is not None and ((long_ and hi >= tp2) or (not long_ and lo <= tp2)):
                return _mk_partial(
                    candles, idx, j, side, entry, tp1, tp2, 0.5, cost_pct, "take"
                )
            if stop is not None and (
                (long_ and lo <= stop) or (not long_ and hi >= stop)
            ):
                return _mk_partial(
                    candles, idx, j, side, entry, tp1, stop, 0.5, cost_pct, "partial_be"
                )
    ...
    return _mk_partial(
        candles, idx, cap, side, entry, p1, exitp, frac, cost_pct, "time_exit"
    )


def _mk_partial(
    candles, idx, exit_idx, side, entry, p1, p2, frac1, cost_pct, outcome
) -> dict[str, Any]:
    ...
    *,
    ...
    for sig in signals:
        ...
        if mode["kind"] == "partial":
            t = _partial_net(
                candles, idx, cap, entry, str(sig["side"]), stop_pct, mode, cost_pct
            )
            if t:
                if (
                    require_complete_horizon
                    and t.get("outcome") == "time_exit"
                ...
                t.update(
                    {
                        ...
                        "quantity_ledger": build_trade_quantity_ledger(
                            closed_legs=(
                                (
                                    float(t["partial_exit_fraction"]),
                                    1.0 - float(t["partial_exit_fraction"]),
                                )
                ...
                trades.append(t)
            continue
        exit_price, outcome, exit_idx = _scan(
            candles, idx, cap, entry, str(sig["side"]), stop_pct, take_pct, mode
        )
        if (
            require_complete_horizon
    ...
    cand = CandidateForValidation.from_dict(
        {
            ...
            "candidate_id": f"exit2::{symbol}::{tf}::{family}",
            "source_run_id": "exit_phase2",
            "symbol": symbol,
            ...
            "metrics": {
                "n_trades": len(trades),
                "data_fingerprint": "selection_only:exit_phase2",
...
def recover_with_phase2(
    ...
    *,
    ...
    path = choose_symbol_file(
        market_data_glob(private_root, timeframe), symbol, timeframe=timeframe
    )
    if not path:
        return {
            "symbol": symbol,
    ...
    variants = {
        name: simulate_exit_mode(candles, signals, params, m) for name, m in modes
    }
    ...
    if not improved:
        ...
        status = (
            _bridge_status(symbol, timeframe, family, params, others[best], len(modes))
            if validate
            else ""
        )
        cls = (
    ...
    return {
        ...
        "outcome_class": cls,
        "all_modes": {
            name: _avg_net(v) for name, v in variants.items()
        },  # every mode logged, not just best
        "paper_forward_ready": False,
    }


def _mean_rev_wrong_exit(
    private_root: Path, *, limit: int | None
) -> list[dict[str, Any]]:
...
def plan_exit_phase2(private_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "wrong_exit_with_params": len(items),
        "by_timeframe": by_tf,
        "exit_modes": [
            n for n, _ in _exit_modes({"stop_pct": 1, "take_pct": 2, "hold_bars": 5})
        ],
    }


def run_exit_phase2(
    private_root: Path, *, limit: int | None = 60, validate: bool = True
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for it in _mean_rev_wrong_exit(private_root, limit=limit):
        res = recover_with_phase2(
            private_root,
            symbol=it["symbol"],
...
def summarize_exit_phase2(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "recovered": by_class.get("exit_recovered_candidate", 0)
        + by_class.get("needs_forward_only", 0),
...
def write_exit_phase2_snapshot(private_root: Path, rows: list[dict[str, Any]]) -> Path:
    ...
    payload = {
        ...
        "by_uc_key": {
            r["uc_key"]: r for r in rows if r.get("uc_key") and "outcome_class" in r
        },
    }
    path.write_text(
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Exit Phase-2 dynamic-exit re-sim (research-only)."
    )
    ap.add_argument(
        "--private-root",
    ...
    ap.add_argument(
        "--no-validate",
        action="store_true",
        help="skip the honest-bridge deflation pass",
    )
    ...
    if args.plan:
        print(
            json.dumps(
                plan_exit_phase2(Path(args.private_root)), ensure_ascii=False, indent=2
            )
        )
        return
    rows = run_exit_phase2(
        Path(args.private_root), limit=args.limit, validate=not args.no_validate
    )
