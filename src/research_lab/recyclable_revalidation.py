...
def revalidate(private_root: Path, *, limit_per_bucket: int | None = None) -> list[dict[str, Any]]:
...
def summarize_revalidation(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "survivors": len(survivors), "survivor_rows": survivors,
        "verdict": ("0 survived honest multiple-testing re-validation - the recyclable shelf is "
                    "characterization, not edge" if not survivors
...
def write_revalidation_snapshot(private_root: Path, rows: list[dict[str, Any]]) -> Path:
    ...
    payload = {
        ...
        "summary": summarize_revalidation(rows), "by_uc_key": by_uc,
    }
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Phase D recyclable re-validation (honest, dry-run).")
    ap.add_argument("--private-root", default=os.getenv("TRADING_BOT_RESEARCH_ROOT", str(DEFAULT_PRIVATE_ROOT)))
    ap.add_argument("--limit-per-bucket", type=int, default=0, help="0 = all")
    ap.add_argument("--snapshot", action="store_true", help="write the derived revalidation artifact")
    args = ap.parse_args()
