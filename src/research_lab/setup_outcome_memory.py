...
def derive_outcome_class(
    lifecycle_state: str, subreason: str, recovered: bool, hard_status: str = ""
) -> str:
...
@dataclass
class GateIndex:
    by_sweep: dict[str, dict[str, Any]] = field(
        default_factory=dict
    )  # sym|tf|fam|fingerprint
    by_cell: dict[str, dict[str, Any]] = field(default_factory=dict)  # sym|tf|fam
    by_params: dict[str, dict[str, Any]] = field(
        default_factory=dict
    )  # sym|tf|fam|params_hash
...
def lookup(
    index: GateIndex, *, symbol: str, timeframe: str, family: str, data_fingerprint: str
) -> MemoryVerdict:
    ...
    if sweep and sweep["n"] > 0:
        ...
        v.reason = (
            "confirmed_bad_identical_data"
            if sweep["n_confirmed_bad"] > 0
    ...
    if cell and cell["n"] > 0:
        if cell["n_eligible"] > 0:
            return MemoryVerdict(
                "revisit",
                "cell_has_eligible_prior",
                "cell",
                cell["n_eligible"],
                cell["n_confirmed_bad"],
                cell["max_n_trades"],
            )
        if (
            cell["n"] >= DEPRIORITIZE_MIN
            and cell["n_rejected"] / cell["n"] >= DEPRIORITIZE_BAD_SHARE
        ):
            return MemoryVerdict(
                "deprioritize",
                "cell_historically_all_rejected",
                "cell",
                0,
                cell["n_confirmed_bad"],
                cell["max_n_trades"],
...
def proposal_verdict(
    index: GateIndex, *, symbol: str, timeframe: str, family: str, params_hash: str
) -> ProposalMemoryVerdict:
    ...
    if (
        exact
        and exact["n"] > 0
        and exact["n_eligible"] == 0
        and exact["n_confirmed_bad"] > 0
    ):
    ...
    if cell and cell["n"] > 0:
        ...
        if (
            ...
            return ProposalMemoryVerdict(
                "known_bad", "cell_confirmed_dead_with_power", "cell"
            )
...
REVISIT_TRIGGERS = (
    "new_data_fingerprint",
    "new_params_or_exit",
    "new_timeframe_horizon",
    "new_oi_funding_context",
    "ttl_expired",
    "manual_go",
)
...
def prior_for_cell(
    index: GateIndex, *, symbol: str, timeframe: str, family: str
) -> dict[str, Any]:
    ...
    if not cell:
        return {}
    return {
        "fingerprints": set(cell.get("fingerprints") or set()),
...
def revisit_policy(
    ...
    *,
    ttl_days: int = DEFAULT_REVISIT_TTL_DAYS,
    ...
    """
    triggers: list[str] = []
    if candidate.get("data_fingerprint") and candidate["data_fingerprint"] not in (
        prior.get("fingerprints") or set()
    ):
        triggers.append("new_data_fingerprint")
    if candidate.get("params_hash") and candidate["params_hash"] not in (
        prior.get("params_hashes") or set()
    ):
        triggers.append("new_params_or_exit")
    if candidate.get("horizon") is not None and candidate.get("horizon") not in (
        prior.get("horizons") or set()
    ):
    ...
    if (
        now_ts is not None
        and newest > 0
        and (int(now_ts) - newest) >= int(ttl_days) * _MS_PER_DAY
    ):
...
def gate_distribution(index: GateIndex) -> dict[str, int]:
    ...
    for key in index.by_sweep:
        try:
            ...
            continue
        action = lookup(
            index, symbol=sym, timeframe=tf, family=fam, data_fingerprint=fp
        ).action
        out[action] = out.get(action, 0) + 1
    return out


def knowledge_base_counts(
    records: list[dict[str, Any]],
    index: GateIndex,
    *,
    survived_shadow: int = 0,
    tactical_probe: int = 0,
) -> dict[str, int]:
...
def memory_prompt_digest(
    records: list[dict[str, Any]], *, max_families: int = 6
) -> str:
    ...
    top_dead = (
        ", ".join(
            f"{k}({v})"
            for k, v in sorted(dead_by_fam.items(), key=lambda kv: -kv[1])[
                :max_families
            ]
        )
        or "none"
...
def _recovered_cells(private_root: Path) -> set[str]:
    ...
    return {
        f"{c.get('symbol')}|{c.get('timeframe')}|{c.get('family')}"
        for c in cands
    ...
    *,
    ...
    for row in evidence_selection["items"]:
        ...
        for candidate_id in candidate_ids:
            _add_training_row(
                by_candidate.setdefault(candidate_id, _empty_training_agg()), row
            )
        if cell_key:
            _add_training_row(by_cell.setdefault(cell_key, _empty_training_agg()), row)
        if profile_cell_key:
            _add_training_row(
                by_geometry_profile_cell.setdefault(
                    profile_cell_key, _empty_training_agg()
                ),
                row,
            )
    selection_summary = {
...
def _outcome_retest_memory(
    ...
    for row in payload.get("items") or []:
        ...
        for target, key, match_scope in keys:
            ...
            scoped_trades = int(scoped.get("best_n_trades") or 0)
            if prior is None or scoped_trades >= int(prior.get("best_n_trades") or 0):
                target[key] = scoped
...
def _training_geometry_profile_key(
    symbol: Any, timeframe: Any, family: Any, profile_id: Any
) -> str:
...
def summarize_product_training_memory(
    ...
    *,
    ...
    for key, agg in by_geometry_profile_cell.items():
        ...
        _merge_training_agg(
            by_geometry_profile.setdefault(profile_id, _empty_training_agg()), agg
        )
    ...
    return {
        ...
        "by_timeframe": {
            k: _training_summary(v) for k, v in sorted(by_timeframe.items())
        },
        "by_geometry_profile": {
            k: _training_summary(v) for k, v in sorted(by_geometry_profile.items())
        },
...
def _merge_training_agg(target: dict[str, Any], source: dict[str, Any]) -> None:
    ...
    for key in ("paper_pnl_usdt", "net_pct_sum"):
        target[key] = round(
            float(target.get(key) or 0.0) + float(source.get(key) or 0.0), 6
        )
...
def _uc_run_dirs(private_root: Path) -> dict[str, str]:
    ...
    try:
        rows = conn.execute(
            "SELECT uc_key, run_dir_label FROM unique_candidates"
        ).fetchall()
    finally:
        conn.close()
    ...
    *,
    ...
    for index, lc in enumerate(lifecycle, start=1):
        ...
        outcome = derive_outcome_class(
            lc["derived_lifecycle_state"], sub_reason, recovered, lc["hard_status"]
        )
        bf = backfill.get(uc, {})
        ...
        tactical_cls = tactical_class(
            lc["n_trades"], lc["avg_net_pct"], lc["hard_status"]
        )
        records.append(
            {
                "uc_key": uc,
                "symbol": lc["symbol"],
                "okx_inst": _okx_inst(lc["symbol"]),
                "timeframe": lc["timeframe"],
                "family": lc["family"],
                "params_hash": lc["params_hash"],
                "data_fingerprint": lc["data_fingerprint"],
                "regime_bucket": lc["regime_bucket"],
                "n_trades": lc["n_trades"],
                ...
                "outcome_retest": retest_by_candidate.get(
                    str(lc.get("source_candidate_id") or "")
                )
                ...
                "lite_status": lc["lite_status"],
                "hard_status": lc["hard_status"],
                "validation_state": lc["validation_state"],
                "tactical_status": lc["tactical_status"],
                "rejection_reason": sub_reason,
                "recovery_reason": "exit_recovered_cell" if recovered else "",
                "outcome_class": outcome,
                "lifecycle_state": lc["derived_lifecycle_state"],
                "paper_forward_ready": bool(lc["paper_forward_ready"]),
                "paper_outcome_count": lc["paper_outcome_count"],
                "artifact_ref": run_dirs.get(uc, ""),
                # Trade-path metrics from the bounded backfill (None until backfill is run for this uc).
                "time_to_mfe": bf.get("avg_time_to_mfe"),
                "time_to_mae": bf.get("avg_time_to_mae"),
                ...
                "revalidation_status": (revalidation.get(uc) or {}).get(
                    "revalidation_status"
                ),
...
def _tally_nested_field(
    records: list[dict[str, Any]], parent: str, field_name: str
) -> dict[str, int]:
    out: dict[str, int] = {}
    for row in records:
        raw_nested = row.get(parent)
...
def summarize_memory(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    total_paper_pnl = round(
        sum(float(r.get("paper_pnl_usdt") or 0.0) for r in paper_rows), 6
    )
    total_paper_terminal = sum(
        int(r.get("paper_terminal_rows") or 0) for r in paper_rows
    )
    return {
        "total": len(records),
        ...
        "paper_avg_pnl_usdt": (
            round(total_paper_pnl / total_paper_terminal, 6)
            if total_paper_terminal
            else 0.0
        ),
        "paper_gave_back_rows": sum(
            int(r.get("paper_gave_back_rows") or 0) for r in paper_rows
        ),
        ...
        "outcome_retest_by_verdict": _tally_nested_field(
            records, "outcome_retest", "verdict"
        ),
        "outcome_retest_by_scope": _tally_nested_field(
            records, "outcome_retest", "match_scope"
        ),
        "one_shot_candidates": sum(
            1 for r in records if r.get("tactical_class") == "one_shot_candidate"
        ),
        "cost_bound_maker_unlock": sum(
            1 for r in records if r.get("cost_class") == "cost_bound_maker_unlock"
        ),
        ...
        "paper_ready_without_hard_pass": sum(
            1
            for r in records
            if r["paper_forward_ready"]
            and r.get("hard_status") != "PAPER_FORWARD_READY"
        ),
    }
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Setup Outcome Memory (derived read-model + gate)."
    )
    ap.add_argument(
        "--private-root",
    ...
    ap.add_argument(
        "--snapshot", action="store_true", help="write the derived snapshot artifact"
    )
    args = ap.parse_args()
