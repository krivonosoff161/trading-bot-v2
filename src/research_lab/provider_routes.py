...
def provider_route_table(environ: Mapping[str, str] | None = None) -> list[dict[str, Any]]:
    ...
    routes = [
        ...
        },
        {
            ...
            "input": "OutcomeLearningCase.v1 + read-only plan/outcome/path facts",
            ...
            "active": _has(env, "ALIBABA_API_KEY"),
            "prompt_version": "outcome_reviewer_v1",
        ...
        },
        {
            ...
            "provider": "alibaba" if _has(env, "ALIBABA_API_KEY") else "disabled",
            "model": str(env.get("LLM_CHEAP_MODEL") or env.get("LLM_MODEL") or "qwen-plus"),
            "input": "ValidatorTaxonomy.v1 + candidate facts",
            "output": "ValidatorReview.v1",
            "cost_logging": "strategy-lab/state/llm_advice/validator_reviews.jsonl + llm_usage",
            "fallback": "deterministic validator taxonomy",
            "active": _has(env, "ALIBABA_API_KEY"),
            "prompt_version": "validator_reviewer_v1",
            "prompt_hash": "role_registry_bound",
        },
        {
            ...
            "provider": "alibaba" if _has(env, "ALIBABA_API_KEY") else "disabled",
            "model": str(env.get("LLM_CHEAP_MODEL") or env.get("LLM_MODEL") or "qwen-plus"),
            "input": "ScannerEvent.v1 + source metadata + later outcome facts",
            "output": "SourceTrustEvent.v1",
            "cost_logging": "strategy-lab/state/llm_advice/source_trust_events.jsonl + llm_usage",
            "fallback": "unknown source trust",
            "active": _has(env, "ALIBABA_API_KEY"),
            "prompt_version": "source_trust_reviewer_v1",
            "prompt_hash": "role_registry_bound",
        },
    ]
    return routes
