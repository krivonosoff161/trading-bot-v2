...
def _accepted_rows(private_root: Path, recipient: str) -> list[dict[str, Any]]:
    ...
    for path in sorted(base.glob("env_*.json")):
        ...
        if not isinstance(task_spec, dict) or task_spec.get("schema") != "RoleTaskSpec.v1":
...
def _retest_specs(private_root: Path) -> list[dict[str, Any]]:
    data = _read_json(resolve_private_child(private_root, "state", "derived", "outcome_retest_specs.json"))
    if not isinstance(data, dict):
        return []
    return [row for row in (data.get("items") or []) if isinstance(row, dict)]


def _paper_rows(private_root: Path) -> list[dict[str, Any]]:
    data = _read_json(resolve_private_child(private_root, "state", "derived", "paper_product_trades.json"))
...
def _dispatch_farm(private_root: Path, tasks: FarmTasksDB, row: dict[str, Any]) -> dict[str, Any]:
    result = _base_result(row, "farm")
    source_ref = str((row.get("task_spec") or {}).get("source_ref") or "")
    spec = next((item for item in _retest_specs(private_root) if str(item.get("source_ref") or "") == source_ref), None)
...
def _dispatch_validator(tasks: FarmTasksDB, row: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate = next(
        (item for item in tasks.latest_unique_candidates(limit=5000)
...
def _dispatch_trader(private_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    result = _base_result(row, "trader")
    subject = (row.get("task_spec") or {}).get("subject") or {}
    symbol = str(subject.get("symbol") or "")
    group = [item for item in _paper_rows(private_root) if str(item.get("symbol") or "") == symbol]
    if not group:
    ...
    replay_path = resolve_private_child(
        private_root, "state", "role_work_results", "trader", f"{row['environment_id']}.json"
    )
    ...
    result.update(status="completed", reason="paper_replay_completed", work_ref=str(replay_path))
    return result


def dispatch_role_environments(
    private_root: Path, tasks: FarmTasksDB, *, apply: bool, limit_per_role: int = 20
) -> dict[str, Any]:
    ...
    for recipient in ("farm", "validator", "trader"):
        ...
        for row in _accepted_rows(private_root, recipient)[: max(0, int(limit_per_role))]:
            counters["seen"] += 1
            if not apply:
                counters["waiting"] += 1
                continue
            if recipient == "farm":
                ...
                result = _dispatch_trader(private_root, row)
            _write_json(_dispatch_path(private_root, recipient, row["environment_id"]), result)
...
def reconcile_role_work_results(
    private_root: Path, tasks: FarmTasksDB, *, apply: bool
) -> dict[str, Any]:
        ...
        for path in sorted(directory.glob("env_*.json")):
            ...
            if recipient == "trader":
                result_path = resolve_private_child(
                    private_root, "state", "role_work_results", "trader", f"{environment_id}.json"
                )
                ...
                if terminal:
                    ...
                    result = {
                        ...
                        "task_type": wanted,
                        "result_ref": str(task.get("last_result_ref") or task.get("run_dir_label") or ""),
