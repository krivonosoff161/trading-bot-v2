...
def fetch_market(report):
    ...
    try:
        ...
        market = {
            ...
            "mcap_change_24h_pct": round(d.get("market_cap_change_percentage_24h_usd", 0.0), 2),
        ...
        report["sources_failed"].append({"src": "coingecko_global", "reason": f"{type(e).__name__}: {e}"})
        return {}
...
def fetch_trending(report):
    ...
    try:
        ...
        report["sources_failed"].append({"src": "coingecko_trending", "reason": f"{type(e).__name__}: {e}"})
        return []
    ...
    try:
        ...
        def slim(r):
            return {
                ...
                "change_24h_pct": (round(r["price_change_percentage_24h"], 2)
        ...
        report["sources_failed"].append({"src": "coingecko_markets", "reason": f"{type(e).__name__}: {e}"})
    return movers
...
def fetch_news(report):
    ...
    for src, url in RSS_FEEDS:
        try:
            ...
            report["sources_failed"].append({"src": f"rss_{src.lower()}", "reason": f"{type(e).__name__}: {e}"})
    return news
...
def fetch_fear_greed(report):
    try:
        ...
        fg = {
            ...
            "ts_utc": dt.datetime.fromtimestamp(int(row["timestamp"]), dt.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        }
        report["sources_ok"].append("fear_greed")
        return fg
    except Exception as e:
        report["sources_failed"].append({"src": "fear_greed", "reason": f"{type(e).__name__}: {e}"})
        return None
...
def fetch_okx(report):
    try:
        d = _get("https://www.okx.com/api/v5/market/tickers", params={"instType": "SWAP"}).json()
        if str(d.get("code")) != "0":
        ...
        def chg_pct(r):
            try:
                last = float(r["last"]); sod = float(r["sodUtc0"])
        ...
        report["sources_failed"].append({"src": "okx_public", "reason": f"{type(e).__name__}: {e}"})
        return {}
...
def render_md(b):
    ...
    L.append(f"- Sources OK ({len(b['sources_ok'])}): {', '.join(b['sources_ok']) or '—'}")
    if b["sources_failed"]:
        fails = ", ".join(f"{f['src']} ({f['reason'].split(':')[0]})" for f in b["sources_failed"])
    ...
    if m:
        ...
        L.append(f"- BTC dominance: {m.get('btc_dominance_pct')}%  |  ETH dominance: {m.get('eth_dominance_pct')}%")
        L.append(f"- Total mcap: {_fmt_usd(m.get('total_mcap_usd'))}  (24h change: {m.get('mcap_change_24h_pct')}%)")
        L.append(f"- Total 24h volume: {_fmt_usd(m.get('total_vol24h_usd'))}")
        L.append(f"- Active cryptos: {m.get('active_cryptocurrencies')}  |  markets: {m.get('markets')}")
        L.append("")

    fg = b.get("fear_greed")
    if fg:
        L.append("## Fear & Greed")
        L.append(f"- {fg.get('value')} / 100 — {fg.get('classification')}  (as of {fg.get('ts_utc')})")
        L.append("")
    ...
    def _tbl(rows):
        for r in rows:
            L.append(f"  - {r.get('symbol'):<8} {str(r.get('change_24h_pct')):>8}%  "
    ...
    if mv.get("trending"):
        ...
        for t in mv["trending"]:
            L.append(f"  - {t.get('symbol'):<8} {t.get('name')}  (rank {t.get('market_cap_rank')})")
        L.append("")

    okx = b.get("okx") or {}
    if okx.get("top_by_turnover"):
        L.append(f"## OKX SWAP top by 24h turnover ({okx.get('n_instruments')} instruments)")
        for r in okx["top_by_turnover"]:
            L.append(f"  - {r.get('instId'):<18} {str(r.get('change_24h_pct')):>8}%  "
...
def main():
    ...
    print(f"sources OK:    {len(bundle['sources_ok'])} -> {', '.join(bundle['sources_ok'])}")
    if bundle["sources_failed"]:
        print(f"sources FAIL:  {len(bundle['sources_failed'])} -> "
              + "; ".join(f"{f['src']}: {f['reason']}" for f in bundle["sources_failed"]))
    else:
        print("sources FAIL:  0")
    if m:
        print(f"market:        BTC.dom={m.get('btc_dominance_pct')}% ETH.dom={m.get('eth_dominance_pct')}% "
    ...
    if mv.get("gainers"):
        g = mv["gainers"][0]
        l = mv["losers"][0] if mv.get("losers") else {}
        print(f"top gainer:    {g.get('symbol')} {g.get('change_24h_pct')}%   "
              f"top loser: {l.get('symbol')} {l.get('change_24h_pct')}%")
    if mv.get("trending"):
